...
//     /*
//     !! DEPRICATED ? descripter.cppの方に新しい関数がある．
//     ある原子種に対する記述子を作成する．
//     input
//     -----------
...
// std::vector<double> raw_get_desc_bondcent_allinone(const Atoms &atoms, Eigen::Vector3d bond_center, int mol_id, std::vector<std::vector<double> > UNITCELL_VECTORS, int NUM_MOL_ATOMS){
//     /*
//     TODO :: const + 参照化
//     ボンドセンター用の記述子を作成
//     TODO : 引数が煩雑すぎるので見直したい．mol_idが必要なのはあまり賢くない．
...
//     TODO :: push_backを使わないで予めDescsの大きさを指定する．（そのためには，raw_get_desc_lonepairの長さ（これは後々変わる可能性がある）を綺麗に知らないといけない．）
//     */
...
// std::vector<int> get_atomslist_from_itpdata(std::vector<int> atoms_list_in_molecule, int NUM_MOL, int NUM_MOL_ATOMS){
//     /*
//     itp_dataでは，1分子内での特定の原子のindexを保持している(o_listなど)．
//     これをNUM_MOL回繰り替えしてconfig全体のindexに変換する関数．
//     */
//     std::vector<int> atoms_list_in_frame(atoms_list_in_molecule.size()*NUM_MOL);
//     for (int i=0;i<NUM_MOL;i++){
//         for (int j=0;j<atoms_list_in_molecule.size();j++){
//             atoms_list_in_frame[i*atoms_list_in_molecule.size() + j] = i*NUM_MOL_ATOMS + atoms_list_in_molecule[j];
//         }
//     }
//     return atoms_list_in_frame;
// }

// std::vector<std::vector<double> > raw_calc_lonepair_descripter_at_frame2(const Atoms &atoms_fr, const std::vector<std::vector<Eigen::Vector3d> > &list_mol_coords, std::vector<int> at_list, int NUM_MOL, std::vector<std::vector<double>> UNITCELL_VECTORS, int NUM_MOL_ATOMS, std::string desctype) {
//     /*
    
//     * @param[in] at_list  :: 記述子を計算したい原子のindex(atoms_frのなかで)． 通常の利用方法だと，たとえばitp_data.o_listなどから一旦作る必要がある．
//     * @param[in] desctype :: 記述子のタイプを指定する（old, allinone）
//     TODO :: push_backを使わないで予めDescsの大きさを指定する．（そのためには，raw_get_desc_lonepairの長さ（これは後々変わる可能性がある）を綺麗に知らないといけない．）
//     */
//     // std::vector<int> at_list2 = raw_find_atomic_index(atoms_fr, atomic_index, NUM_MOL);
//     if (desctype == "old"){
//         std::cout << "ERROR in raw_calc_lonepair_descripter_at_frame2 : desctype = old is not supported." << std::endl;
//         exit(1);
//     }

//     std::vector<std::vector<double> > Descs;
//     std::vector<Eigen::Vector3d> list_lonepair_coords(at_list.size());
    
//     int mol_index, atom_index;

//     if (at_list.size() != 0) { // at_listが非ゼロなら記述子計算を実行
//         for (auto index : at_list) {
//             mol_index  = index / NUM_MOL_ATOMS;
//             atom_index = index % NUM_MOL_ATOMS;
//             Descs.push_back(raw_get_desc_lonepair_allinone(atoms_fr, list_mol_coords[mol_index][atom_index], UNITCELL_VECTORS, NUM_MOL_ATOMS));
//         }
//     }
//     return Descs;
// }
