...
from mlwc.dataset.ethanol.dataset_aseatom_eth import (
    ...
    ethanol_ch_descriptor,
    ethanol_co_descriptor,
    ethanol_oh_descriptor,
    ethanol_olp_descriptor,
    ethanol_pbc_atoms,
)
from mlwc.dataset.methanol.dataset_aseatom_met import (
    ...
    methanol_bc,
    methanol_bond,
    methanol_ch_descriptor,
    methanol_co_descriptor,
...
def test_bondcenter_met(
    ...
    # Olp
    bond_centers = methanol_atoms.get_positions()[
        np.argwhere(methanol_atoms.get_atomic_numbers() == 8).reshape(-1)
    ]
    descs_olp = DESC.calc_descriptor(
        atoms=methanol_atoms,
        bond_centers=bond_centers,
        list_atomic_number=[6, 1, 8],
        list_maxat=[24, 24, 24],
...
def test_bondcenter_eth(
    ...
    # Olp
    bond_centers = ethanol_atoms.get_positions()[
        np.argwhere(ethanol_atoms.get_atomic_numbers() == 8).reshape(-1)
    ]
    descs_olp = DESC.calc_descriptor(
        atoms=ethanol_atoms,
        bond_centers=bond_centers,
