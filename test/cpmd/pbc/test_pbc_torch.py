...
def test_pbc_2d_torch():
    ...
    vectors_array = torch.randn(
        10, 3, dtype=torch.float32, requires_grad=True
    )  # 10個の3次元ベクトル
    ...
    assert (
        output.shape == vectors_array.shape
    ), f"Expected shape {vectors_array.shape}, but got {output.shape}"

    # grad check
    vectors_array = vectors_array.clone().requires_grad_(True)
    cell = cell.clone()
    assert torch.autograd.gradgradcheck(
        model, (vectors_array, cell), eps=1e-3, atol=1e-4, rtol=1e-3
...
def test_pbc_3d_torch():
    ...
    # 2. 勾配計算が可能かどうかの確認
    assert gradcheck(
        model, (vectors_array.double().requires_grad_(), cell.double().requires_grad_())
    )
