...
from mlwc.dataset.pg2.dataset_aseatom_pg2 import (
    pg2_atoms,
    pg2_bc,
    pg2_bond,
    pg2_pbc_atoms,
)
from mlwc.dataset.pg12.dataset_aseatom_pg12 import (
    pg12_1mol_atoms,
    pg12_atoms,
    pg12_bc,
    pg12_bond,
...
from mlwc.dataset.pg.dataset_aseatom_pg import pg_atoms, pg_bc, pg_bond, pg_pbc_atoms
from mlwc.dataset.pmma.dataset_aseatom_pmma import (
    pmma_atoms,
    pmma_bc,
    pmma_bond,
    pmma_pbc_atoms,
...
def test_raw_get_distances_mic_without_mic(water_molecule, water_molecule_bond):
    ...
    # 実行
    calculated_vectors = pbc_mol.compute_pbc(
        water_molecule.get_positions(),
    ...
    np.testing.assert_almost_equal(
        calculated_vectors.reshape(-1, 3), expected_positions, decimal=5
    )
...
def test_raw_get_distances_mic_with_mic(water_molecule, water_molecule_bond):
    ...
    # 実行
    result_atoms = pbc_mol.compute_pbc(
        water_molecule.get_positions(),
        np.array(water_molecule.get_cell()),
        bondlist,
        NUM_MOL_PAR_MOL,
...
def test_raw_get_distances_mic_empty_mol_inds(water_molecule, water_molecule_bond):
    ...
    # 結果が元の位置と一致するか確認
    result_vectors = pbc_mol.compute_pbc(
        water_molecule.get_positions(),
        np.array(water_molecule.get_cell()),
        bondlist,
        NUM_MOL_PAR_MOL,
    ...
    expected_positions = water_molecule.get_positions()
    np.testing.assert_almost_equal(
        result_vectors.reshape(-1, 3), expected_positions, decimal=5
    )
...
def test_pbc_mol_met(ethanol_atoms, ethanol_bond, ethanol_pbc_atoms):
    ...
    # 結果の検証: 移動後の位置が元の位置と一致することを確認
    np.testing.assert_almost_equal(
        calculated_vectors.reshape(-1, 3), ethanol_pbc_atoms, decimal=5
    )
...
def test_pbc_mol_pg(pg_atoms, pg_bond, pg_pbc_atoms):
    ...
    # 結果の検証: 移動後の位置が元の位置と一致することを確認
    np.testing.assert_almost_equal(
        calculated_vectors.reshape(-1, 3), pg_pbc_atoms, decimal=5
    )
...
def test_pbc_mol_pg2(pg2_atoms, pg2_bond, pg2_pbc_atoms):
    ...
    # 結果の検証: 移動後の位置が元の位置と一致することを確認
    np.testing.assert_almost_equal(
        calculated_vectors.reshape(-1, 3), pg2_pbc_atoms, decimal=5
    )
...
def test_pbc_mol_pg12(pg12_atoms, pg12_bond, pg12_pbc_atoms):
    ...
    # 結果の検証: 移動後の位置が元の位置と一致することを確認
    np.testing.assert_almost_equal(
        calculated_vectors.reshape(-1, 3), pg12_pbc_atoms, decimal=5
    )
...
def test_pbc_mol_pg12_1mol(pg12_1mol_atoms, pg12_bond, pg12_pbc_atoms):
    ...
    # 結果の検証: 移動後の位置が元の位置と一致することを確認
    np.testing.assert_almost_equal(
        calculated_vectors.reshape(-1, 3), pg12_pbc_atoms[123 : 2 * 123], decimal=5
    )
...
def test_pbc_mol_pg12_1mol_2(pg12_atoms, pg12_bond, pg12_pbc_atoms):
    ...
    for mol_id in range(10):
        ...
        # 結果の検証: 移動後の位置が元の位置と一致することを確認
        np.testing.assert_almost_equal(
            calculated_vectors.reshape(-1, 3),
            pg12_pbc_atoms[mol_id * 123 : (mol_id + 1) * 123],
            decimal=5,
        )
...
def test_pbc_mol_pg12_1mol_3(pg12_atoms, pg12_bond, pg12_pbc_atoms):
    ...
    for mol_id in range(1):
        ...
        np.testing.assert_almost_equal(
            calculated_vectors.reshape(-1, 3),
            pg12_pbc_atoms[mol_id * 123 : (mol_id + 3) * 123],
            decimal=5,
        )
...
def test_pbc_mol_pmma(pmma_atoms, pmma_bond, pmma_pbc_atoms):
    ...
    # 結果の検証: 移動後の位置が元の位置と一致することを確認
    np.testing.assert_almost_equal(
        calculated_vectors.reshape(-1, 3), pmma_pbc_atoms, decimal=5
    )
