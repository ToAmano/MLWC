...
from mlwc.dataset.ethanol.dataset_aseatom_eth import (
    ethanol_atoms,
    ethanol_bc,
    ethanol_bond,
)
...
from mlwc.dataset.pg2.dataset_aseatom_pg2 import pg2_atoms, pg2_bc, pg2_bond
from mlwc.dataset.pg12.dataset_aseatom_pg12 import pg12_atoms, pg12_bc, pg12_bond
from mlwc.dataset.pg.dataset_aseatom_pg import pg_atoms, pg_bc, pg_bond
from mlwc.dataset.pmma.dataset_aseatom_pmma import pmma_atoms, pmma_bc, pmma_bond
...
def test_bondcenter_met(methanol_atoms, methanol_bond, methanol_bc):
    ...
    np.testing.assert_almost_equal(
        list_bond_centers.reshape(-1, 3), methanol_bc, decimal=5
    )
    # calculate bond centers
    dict_bond_centers = calc_bondcenter_dict(list_mol_coords, bonds)
    np.testing.assert_almost_equal(
        dict_bond_centers["OH_1_bond"].reshape(-1, 3),
        methanol_bc.reshape(-1, 5, 3)[:, 4, :].reshape(-1, 3),
        decimal=5,
    )
    np.testing.assert_almost_equal(
        dict_bond_centers["CO_1_bond"].reshape(-1, 3),
        methanol_bc.reshape(-1, 5, 3)[:, 3, :].reshape(-1, 3),
        decimal=5,
    )
    np.testing.assert_almost_equal(
        dict_bond_centers["CH_1_bond"].reshape(-1, 3),
        methanol_bc.reshape(-1, 5, 3)[:, [0, 1, 2], :].reshape(-1, 3),
        decimal=5,
    )
...
def test_bondcenter_eth(ethanol_atoms, ethanol_bond, ethanol_bc):
    ...
    # 結果の検証: 移動後の位置が元の位置と一致することを確認
    np.testing.assert_almost_equal(
        list_bond_centers.reshape(-1, 3), ethanol_bc, decimal=5
    )


def test_bondcenter_pg(pg_atoms, pg_bond, pg_bc):
    bondlist, NUM_MOL_PAR_MOL, ref_atom_index = pg_bond

    list_mol_coords = pbc_mol.compute_pbc(
        pg_atoms.get_positions(),
    ...
    list_bond_centers = calc_bondcenter(list_mol_coords, bondlist)
    # 結果の検証: 移動後の位置が元の位置と一致することを確認
    np.testing.assert_almost_equal(list_bond_centers.reshape(-1, 3), pg_bc, decimal=5)
...
def test_bondcenter_pmma(pmma_atoms, pmma_bond, pmma_bc):
    bondlist, NUM_MOL_PAR_MOL, ref_atom_index = pmma_bond

    list_mol_coords = pbc_mol.compute_pbc(
        pmma_atoms.get_positions(),
    ...
    list_bond_centers = calc_bondcenter(list_mol_coords, bondlist)
    # 結果の検証: 移動後の位置が元の位置と一致することを確認
    np.testing.assert_almost_equal(list_bond_centers.reshape(-1, 3), pmma_bc, decimal=5)
