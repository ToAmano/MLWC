...
def plot_loss_class(test, train):
    ...
    scatter1 = ax.scatter(np.arange(len(test)), test,
                          label="test", alpha=0.2, color="#1f77b4", s=5)  # 描画
    scatter2 = ax.scatter(np.arange(len(train)), train,
                          label="train", alpha=0.2, color='#ff7f0e', s=5)
...
def plot_residure_train_valid(model, dataset_train, dataset_valid, limit: bool = True):
    ...
    import matplotlib.pyplot as plt
    import numpy as np
    from ml.train.ml_train import calculate_final_dipoles

    # [size,3]の形でdipoleを返す
    train_pred_list, train_true_list = calculate_final_dipoles(
        model, dataset_train)
    valid_pred_list, valid_true_list = calculate_final_dipoles(
        model, dataset_valid)
    ...
    ax1.scatter(
        train_pred_list[:, 0], train_true_list[:, 0], alpha=0.03, s=1, label="train")
    ax1.scatter(
        valid_pred_list[:, 0], valid_true_list[:, 0], alpha=0.03, s=1, label="valid")

    ax2.scatter(
        train_pred_list[:, 1], train_true_list[:, 1], alpha=0.03, s=1, label="train")
    ax2.scatter(
        valid_pred_list[:, 1], valid_true_list[:, 1], alpha=0.03, s=1, label="valid")

    ax3.scatter(
        train_pred_list[:, 2], train_true_list[:, 2], alpha=0.03, s=1, label="train")
    ax3.scatter(
        valid_pred_list[:, 2], valid_true_list[:, 2], alpha=0.03, s=1, label="valid")
...
def plot_residure(y_pred_train, y_pred_test, true_y, test_y, limit: bool = True):
    ...
    for i, c in enumerate(vector):
        ...
        rmse_test = np.sqrt(np.mean((x1[:, i]-y1[:, i])**2))
        print(
            "rmse(train):  {0}  / rmse(test):  {1}".format(rmse_train, rmse_test))
...
def plot_norm_from_data(y_pred_train, y_pred_valid, y_true_train, y_true_valid, limit: bool = True, save="", title: str = "ML_result"):
    ...
    # figure, axesオブジェクトを作成
    fig, ax = plt.subplots(figsize=(8, 5), tight_layout=True)
    ax.scatter(np.linalg.norm(y_pred_train, axis=1), np.linalg.norm(
        y_true_train, axis=1), alpha=0.2, s=5, label="train RMSE={}".format(rmse0))
    ax.scatter(np.linalg.norm(y_pred_valid, axis=1), np.linalg.norm(
        y_true_valid, axis=1), alpha=0.2, s=5, label="test RMSE={}".format(rmse1))
    if limit:
        ax.xlim(0, 4)
        ax.ylim(0, 4)
    # 各要素で設定したい文字列の取得
...
def plot_norm(model, dataset_train, dataset_valid, limit: bool = True):
    ...
    # figure, axesオブジェクトを作成
    fig, ax = plt.subplots(figsize=(8, 5), tight_layout=True)
    scatter1 = ax.scatter(np.linalg.norm(train_pred_list, axis=1), np.linalg.norm(
        train_true_list, axis=1), alpha=0.2, s=5, label="train RMSE={}".format(rmse_train))
    scatter2 = ax.scatter(np.linalg.norm(valid_pred_list, axis=1), np.linalg.norm(
        valid_true_list, axis=1), alpha=0.2, s=5, label="test RMSE={}".format(rmse_valid))
