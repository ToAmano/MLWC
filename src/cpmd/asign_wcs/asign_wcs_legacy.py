...
import sys
import numpy as np
from bond.atomtype import Node  # 深さ優先探索用
from bond.atomtype import raw_make_graph_from_itp  # 深さ優先探索用
from collections import deque  # 深さ優先探索用
# from types import NoneType
import torch
from cpmd.distance.distance import distance_matrix
...
def relative_vector_torch(atom1, atom2, UNITCELL_VECTORS: np.array):
    ...
    fractional_relative_vec = fractional_relative_vec - \
        torch.round(fractional_relative_vec)
...
def relative_vectors_torch2(atom1, atom2, UNITCELL_VECTORS):
    ...
    inv_lattice_vectors = torch.linalg.inv(UNITCELL_VECTORS)
    fractional_relative_vecs = torch.matmul(
        relative_vecs, inv_lattice_vectors.T)

    # Apply minimum image convention
    fractional_relative_vecs = fractional_relative_vecs - \
        torch.round(fractional_relative_vecs)
...
def find_nearest_wfc(bondcenters, wfc_list, UNITCELL_VECTORS, num_wcs: int):
    ...
    # TODO :: apply pbc
    diff = distance_matrix.compute_distances(
        bondcenters, wfc_list, UNITCELL_VECTORS)
...
def calc_bondmu():
    ...
    if wcs_num == 1:
        ...
        if np.linalg.norm(wfc_vectors[wcs_indices[0]]) > 1.0:
            print("WARNING :: The distance between atom {} and lonepair {} ({}) is too large !! :: {} ".format(
                atom_coord, wfc_list[wcs_indices[0]], wcs_indices[0], np.linalg.norm(wfc_vectors[wcs_indices[0]])))
        if wcs_indices[0] in picked_wfcs:
            print("WARNING SAME wcs is assined !! :: {}".format(
                wcs_indices[0]))
    if wcs_num == 2:
        ...
        mu_lp = (-4.0)*coef * \
            (wfc_vectors[wcs_indices[0]]+wfc_vectors[wcs_indices[1]])/2.0
...
class asign_wcs_torch:
    ...
    def __init__(self, NUM_MOL: int, NUM_MOL_ATOMS: int, UNITCELL_VECTORS):
        ...
        self.UNITCELL_VECTORS = torch.from_numpy(
            UNITCELL_VECTORS.astype(np.float32)).clone()  # 単位胞ベクトル
    ...
    def _find_nearest_lonepairs(self, atom_coord: np.array, wfc_list, wcs_num: int, UNITCELL_VECTORS, picked_wfcs):
        ...
        atom_wan = raw_make_aseatoms_from_wc(
            atom_coord, wfc_list, UNITCELL_VECTORS)

        # 先頭原子(atom_coord)とWCsの距離ベクトル（PBC，mirror imageを考慮）
        wfc_vectors = relative_vector_torch(
            atom_coord, wfc_list, UNITCELL_VECTORS)
        wfc_distances = torch.linalg.norm(wfc_vectors, axis=1)

        if wcs_num == 1:
            wcs_indices = torch.argsort(
                wfc_distances).reshape(-1)[:1]  # 最も近いWCsのインデックスを一つ取り出す．
            mu_lp = (-2.0)*coef*wfc_vectors[wcs_indices[0]]
            # 原子とwcsの距離があまりに遠い場合はWARNINGを出す
            if np.linalg.norm(wfc_vectors[wcs_indices[0]]) > 1.0:
                print("WARNING :: The distance between atom {} and lonepair {} ({}) is too large !! :: {} ".format(
        ...
        wcs_lp = [atom_wan.get_positions()[0]+wfc_vectors[i]
...
def raw_aseatom_to_mol_coord_bc(ase_atoms, bonds_list, itp_data, NUM_MOL_ATOMS: int, NUM_MOL: int):
    ...
    for j in range(NUM_MOL):
        ...
        mol_coords, bond_centers = raw_calc_mol_coord_and_bc_mic_onemolecule_new(
            mol_inds, bonds_list, ase_atoms[NUM_MOL_ATOMS*j:NUM_MOL_ATOMS*(j+1)], itp_data)  # 1つの分子のmic座標/bond center計算
...
def raw_get_distances_mic(aseatom, a: int, indices, mic=False, vector=False):
    ...
    if mic == True:  # micの時だけdistancesを計算しなおす．
        ...
        distances = np.where(np.abs(distances) > cell/2,
                             distances-cell*np.sign(distances), distances)
    if vector == True:
        return distances
    else:
...
def raw_get_pbc_mol(aseatom, mol_inds, bonds_list_j, itp_data):
    ...
    vectors = raw_get_distances_mic(
        aseatom, mol_inds[itp_data.representative_atom_index], mol_inds, mic=True, vector=True)
    vectors_old = vectors
    ...
    bonds_list_from_zero = [[i[0]-mol_inds[0], i[1]-mol_inds[0]]
                            for i in bonds_list_j]  # TODO :: bonds_list_jから単に数字をシフトする場合にのみ対応
    ...
    if CALC_FLAG == True:
        print("WARNING(raw_get_pbc_mol) :: mol_index {} :: recalculation of vectors is required.".format(
            mol_inds[0]))
        vectors = raw_bfs(aseatom, raw_make_graph_from_itp(
            itp_data), vectors, mol_inds, itp_data.representative_atom_index)
        ...
        for bond in bonds_list_from_zero:  # 全てのボンドに対するloopでvectorsが正しく再計算されたかチェック
            ...
            if bond_distance > 3.0:  # angstroml
                print(" !!ERROR!!: bond distance is too long after BFS modification. bond distance = {0} Angstrom between atom {1}/{2}/{3} and atom {4}/{5}/{6}".format(bond_distance, bond[0], aseatom.get_positions()[
                      bond[0]+mol_inds[0]], aseatom.get_chemical_symbols()[bond[0]+mol_inds[0]], bond[1], aseatom.get_positions()[bond[1]+mol_inds[0]], aseatom.get_chemical_symbols()[bond[1]+mol_inds[0]]))
                print(
                    " vectors[bond[0]]-vectors[bond[1]] {} {}".format(vectors[bond[0]], vectors[bond[1]]))
                print(" ")
                # sys.exit(1)
    return vectors
        ...
        for near in nears:
            if nodes[near].parent == -1:  # 親ノードが-1なら未探索
                ...
                revised_vector = raw_get_distances_mic(
                    aseatom, node.index+mol_inds[0], nodes[near].index+mol_inds[0], mic=True, vector=True)
                ...
                vectors[nodes[near].index] = vectors[node.index] + \
                    revised_vector  # vectorsはrepresentativeからの距離
...
def raw_calc_mol_coord_and_bc_mic_onemolecule_new(mol_inds, bonds_list, aseatoms, itp_data):
    ...
    )[mol_inds[itp_data.representative_atom_index]]  # 最初の原子の座標
    ...
    # 全てのボンドセンターの座標を取得する．
    # 二つのdrがボンドの両端の原子への距離
    bond_centers = [R0+(vectors[l[0]]+vectors[l[1]]) /
                    2.0 for l in bonds_list]  # R0にボンドセンターへの座標をたす．
...
def raw_calc_bc_mic_onemolecule(mol_inds, mol_coords, bonds_list_j):
    ...
    bond_centers = [(mol_coords[l[0]]+mol_coords(l[1])) /
                    2.0 for l in bonds_list_from_zero]
...
def raw_find_lonepairs(atom_coord: np.array, wfc_list, wcs_num: int, UNITCELL_VECTORS, picked_wfcs):
    ...
    wfc_vectors = raw_get_distances_mic_multiPBC(atom_wan, 0, range(
        1, num_element), mic=True, vector=True)  # 上のコードと同じことをしている．
...
def raw_find_bondwcs(atom_coord: np.array, wfc_list, wcs_num: int, UNITCELL_VECTORS):
    ...
    if wcs_num == 1:
        ...
        if np.linalg.norm(wfc_vectors[wcs_indices[0]]) > 1.0:
            print("WARNING :: The distance between bc {} and wc {} is too large !! :: {} Ang.".format(
                atom_coord, wcs_indices[0], np.linalg.norm(wfc_vectors[wcs_indices[0]])))
    if wcs_num == 2:
        # 最も近いWCsのインデックスを二つ取り出す．
        wcs_indices = np.argsort(wfc_distances).reshape(-1)[:2]
        # 二つのWannierCenterによる双極子モーメントを計算する．
        mu_lp = (-4.0)*coef * \
            (wfc_vectors[wcs_indices[0]]+wfc_vectors[wcs_indices[1]])/2.0
    # 最後にwcsの（micがかかった）座標を取得
    wcs_bond = [atom_wan.get_positions()[0]+wfc_vectors[i]
                for i in wcs_indices]
    # for i in wcs_indices:
...
def raw_find_pi(atom_coord: np.array, wfc_list, r_threshold: float, picked_wcs, UNITCELL_VECTORS):
    ...
    wfc_vectors = raw_get_distances_mic_multiPBC(atom_wan, 0, range(
        1, num_element), mic=True, vector=True)  # 上のコードと同じことをしている．
    wfc_distances = np.linalg.norm(wfc_vectors, axis=1)
    wcs_indices = np.argwhere(
        wfc_distances < r_threshold).reshape(-1)  # まずは条件に合うのを抽出
    ...
    if len(wcs_indices) == 0:
        ...
        mu_lp = (-2.0)*coef*wfc_vectors[wcs_indices[0]]
        # 最後にwcsの（micがかかった）座標を取得
        wcs_dbond = [atom_wan.get_positions()[0]+wfc_vectors[i]
                     for i in wcs_indices]
        # for i in wcs_indices:
...
def raw_calc_mu_bond_lonepair(wfc_list, ase_atoms: ase.Atoms, bonds_list, itp_data, double_bonds, NUM_MOL_ATOMS: int, NUM_MOL: int, UNITCELL_VECTORS):
    ...
    atO_list = [np.argwhere(js == 8).reshape(-1)
                for js in list_atomic_nums]  # 原子番号8
    atN_list = [np.argwhere(js == 7).reshape(-1)
                for js in list_atomic_nums]  # 原子番号7
...
def save_yaml(NUM_MOL: int, itpdata, list_bond_centers, list_mu_bonds):
    ...
    for mol_id in range(NUM_MOL):
        ...
        for bond_index, bond in enumerate(itpdata.bonds_list):
            ...
            # ボンドリスト作成
            test[bond_index] = {"bc": cent_mol,
                                "bd": bond_dipole, "index": bond}
            print("")
