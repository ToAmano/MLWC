...
from cpmd.distance import distance_ase, distance_2d
    ...
    while queue:
        ...
        for near in nears:
            if nodes[near].parent == -1: # 親ノードが-1なら未探索
                ...
                revised_vector = distance_2d.compute_distances(vectors[node.index], vectors[nodes[near].index], cell=cell, pbc=True)
...
def check_if_bondlength_large(vectors, bonds_list)->bool:
...
class pbc_mol(pbc_abstract):
    ...
    def compute_pbc(cls,vectors_array:np.ndarray,
        ...
        # graph of molecules
        mol_nodes = make_bondgraph(bonds_list,NUM_ATOM_PAR_MOL)
        ...
        for mol_id in range(NUM_MOL):
            ...
            if check_if_bondlength_large(mol_vectors,bonds_list):
                ...
                # そのような分子に対しては，BFSを行い，原子間距離を再計算する．
                mol_vectors = raw_bfs(mol_vectors, mol_nodes, cell, ref_atom_index)
                # TODO !! check bond length again # check_if_bondlength_large(mol_vectors,bonds_list):
