...
class make_cpmdinput():
     ...
     def __init__(self, ase_atoms,max_step:float=10000,timestep:float=40):
         # get supercell size
 ...
 &CPMD
  ...
  ! SD,JACOBI,SVD
   WANNIER OPTIMIZATION JACOBI
         ...
         &ATOMS
         '''.format(max_step,timestep,cell_parameter)
 ...
 &CPMD
  ...
  &END
     ...
     def make_bomd_restart(self):
          # 
          filename_bomd="bomd-wan-restart.inp"

          f_bomd=open(filename_bomd,mode="w")
          f_bomd.write(self.lines_bomd_wan_restart)
          self.write_coordinates(f_bomd) # 座標を出力
          f_bomd.write("&END \n")
          f_bomd.close()
          return 0
     ...
     def make_cpmd(self):
          ...
          f_bomd=open(filename_bomd,mode="w")
          f_bomd.write(self.lines_cpmd_wan)
          self.write_coordinates(f_bomd) # 座標を出力
          f_bomd.write("&END \n")
          f_bomd.close()
          return 0
...
class make_cpmdinput_type2():
     ...
     そのような場合にはtype2を試して欲しい．
     IONS+CENTERS.xyzに対する後処理コードも準備する予定．
     '''
