...
def raw_aseatom_to_mol_coord_bc(ase_atoms, bonds_list, itp_data, NUM_MOL_ATOMS: int, NUM_MOL: int):
    ...
    mol_ats = [[int(at+NUM_MOL_ATOMS*indx) for at in mol_at0]
               for indx in range(NUM_MOL)]
    # * ? bond indexもJ番目のボンドに対応させる
    unit_cell_bonds = []
    for indx in range(NUM_MOL):
        unit_cell_bonds.append([[int(b_pair[0]+NUM_MOL_ATOMS*indx),
                               int(b_pair[1]+NUM_MOL_ATOMS*indx)] for b_pair in bonds_list])
    ...
    for j in range(NUM_MOL):  # 全ての分子に対する繰り返し．
        ...
        mol_coords, bond_centers = raw_calc_mol_coord_and_bc_mic_onemolecule(
            mol_inds, bonds_list_j, ase_atoms, itp_data)  # 1つの分子のmic座標/bond center計算
...
def get_desc_bondcent_yamazaki(atoms, Rcs, Rc, MaxAt, bond_center, mol_id):
    ...
    s = np.where(d < Rcs, 1/d, np.where(d < Rc, (1/d) *
                 (0.5*np.cos(np.pi*(d-Rcs)/(Rc-Rcs))+0.5), 0))
...
def raw_calc_mol_coord_and_bc_mic_onemolecule(mol_inds, bonds_list_j, aseatoms: ase.Atoms, itp_data):
    ...
    distance_vectors = raw_get_pbc_mol(
        aseatoms, mol_inds, bonds_list_j, itp_data)
    ...
    # 全てのボンドセンターの座標を取得する．
    bond_centers = [R0+(distance_vectors[l[0]] +
                        distance_vectors[l[1]])/2.0 for l in bonds_list_from_zero]
...
def raw_find_pi(atom_coord: np.array, wfc_list, r_threshold: float, picked_wcs, UNITCELL_VECTORS):
    ...
    # 先頭原子とWCsの距離ベクトル
    # wfc_vectors = atom_wan.get_distances(0,range(1,num_element),mic=True,vector=True) #ワニエ中心は一度CP.xを通しているから点ごとにPBCが適用されている。mic=Trueでよい。
    wfc_vectors = raw_get_distances_mic(atom_wan, 0, range(
        1, num_element), mic=True, vector=True)  # 上のコードと同じことをしている．
    wfc_distances = np.linalg.norm(wfc_vectors, axis=1)
