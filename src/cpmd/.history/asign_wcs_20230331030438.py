...
#
# * 
def calc_mu_bond_lonepair(wfc_list,parsed_results) :
    '''
    ...
    # Oローンペア
    list_mu_lpO, list_lpO_wfcs, picked_wcs_O = find_all_lonepairs(wfc_list,atO_list,list_mol_coords,picked_wfcs,wcs_num=2)
    picked_wfcs = picked_wfcs + list(picked_wcs_O)

    # Nローンペア
    list_mu_lpN, list_lpN_wfcs, picked_wcs_N = find_all_lonepairs(wfc_list,atO_list,list_mol_coords,picked_wfcs,wcs_num=1)
    picked_wfcs = picked_wfcs + list(picked_wcs_N)

    # ボンドセンター    
    list_mu_bonds, list_bond_wfcs, picked_wcs_bond = find_all_bonds(wfc_list,list_bond_centers,picked_wfcs)    
    picked_wfcs = picked_wfcs + list(picked_wcs_bond)

    ##########π電子を探索する###########    
    # TODO :: hard code :: ここは改善の余地あり．
    list_mu_pai, list_pi_wfcs, picked_wcs_pi = find_all_pi(wfc_list,list_bond_centers,picked_wfcs) 
