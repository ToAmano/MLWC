...
def raw_find_all_lonepairs(wfc_list,atO_list,list_mol_coords,picked_wfcs,wcs_num:int):
    '''
    ...
    for atOs,mol_coords in zip(atO_list,list_mol_coords) : #分子の数に関するループ
        ...
        for atO in atOs : #ある分子内の原子に関するループ
            ...
            wcs_indices, mu_lp, wcs_lp = raw_find_lonepairs(center_atom_coord, wfc_list, wcs_num)
    ...
    list_mu_lp=np.array(list_mu_lp)
    list_lp_wfcs=np.array(list_lp_wfcs)  
    return list_mu_lp, list_lp_wfcs, picked_wfcs

def raw_find_all_bonds(wfc_list,list_bond_centers,picked_wfcs):
    '''
    シングルボンドの部分を計算
    '''
    list_mu_bonds = []
    ...
    for bcs in list_bond_centers :  # 分子数に関するループ
        ...
        for bond_center_coord in bcs : # ある分子内のボンドセンターに関するループ
            # wfc_list_exclude_pickedwfcs = [wfc_list[config][i] for i in range(len(wfc_list[config])) if i not in picked_wfcs]
            wcs_indices, mu_bond,wcs_bond = raw_find_lonepairs(bond_center_coord, wfc_list, 1)
            picked_wfcs        = picked_wfcs + list(wcs_indices)
            wcs_mol.append(wcs_bond)
            mu_bonds_mol.append(mu_bond)
        list_mu_bonds.append(mu_bonds_mol)
        list_bond_wfcs.append(wcs_mol)
    list_mu_bonds=np.array(list_mu_bonds)
    list_bond_wfcs=np.array(list_bond_wfcs)
    return list_mu_bonds, list_bond_wfcs, picked_wfcs

def raw_find_all_pi(wfc_list,list_bond_centers,picked_wfcs):
    ...
    for bcs in list_bond_centers : # 再度ボンドセンターを探索
        ...
        for dbond_center_coord  in dbcs : # 二重結合的に探索する？
            ...
            # wfc_list_exclude_pickedwfcs = [wfc_list[config][i] for i in range(len(wfc_list[config])) if i not in picked_wfcs]
            wcs_indices, mu_bond, wcs_bond = raw_find_pi(dbond_center_coord,wfc_list,r_threshold,picked_wfcs)
    ...
    list_mu_pai=np.array(list_mu_pai)
    list_pi_wfcs=np.array(list_pi_wfcs)
    return list_mu_pai, list_pi_wfcs, picked_wfcs
