...
class Plot_totaldipole:
    ...
    def __init__(self, dipole_filename):
        ...
        if not os.path.isfile(self._filename):
            logger.error(f" ERROR (Plot_totaldipole) :: {str(self._filename)} does not exist !!")
            logger.error(" ")
            raise FileNotFoundError(f" ERROR (Plot_totaldipole) :: {str(self._filename)} does not exist !!")
        logger.info(" ============================ ")
    ...
    def calc_dielectric_spectrum(self, eps_n2: float, start: int, end: int, step: int, window: str = "hann", if_fft: bool = True):
        ...
        total_dipole = totaldipole().from_array(
...
def command_diel_spectra(args):
    ...
    EVP.calc_dielectric_spectrum(float(args.eps), int(args.start), int(
        args.end), int(args.step), args.window, args.fft)  # epsを受け取ってfloat変換
    ...
    df_diel:pd.Dataframe = total_dipole.calculate_dielfunction(
        eps_inf=float(args.eps), method_dipole="direct",window_type=args.window)
    df_refractive = total_dipole.calculate_refractiveindex(eps_inf=float(args.eps), method_dipole="direct",window_type=args.window)
    # refractive index
    diel = diel_function(df_diel["freq_thz"], df_diel["diel_real"], df_diel["diel_imag"], int(args.step))
