...
def fit_diel(freq: np.ndarray, imag_diel: np.ndarray, num_hn_functions: int = 1, lower_bound: float = 0.1, upper_bound: float = 1.0):
    ...
    # 最小二乗法によるフィッティングを実行
    result = least_squares(fourier.fit_diel.residuals, initial_guess, bounds=(
        bounds_lower, bounds_upper), args=(freq, imag_diel))
    logger.info(" ====================== ")
...
def command_diel_mol(args):
    ...
    EVP.calc_dielectric_spectrum(float(args.eps), int(
        args.start), int(args.end), int(args.step))  # epsを受け取ってfloat変換
    return 0


def command_diel_fit(args):
    df = pd.read_csv(args.Filename)
    fit_diel(df["freq_kayser"].values, df["imag_diel"].values,
             args.num_hn_functions, args.lower_bound, args.upper_bound)
    return 0
