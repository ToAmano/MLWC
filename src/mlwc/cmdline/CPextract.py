...
def parse_cml_args(cml):
    ...
    parser_cp_evp.add_argument("Filename", help="CP.x *.evp file to be parsed.\n")
    ...
    parser_cp_dfset.add_argument("Filename", help="CP.x *.pos file to be parsed.\n")
    parser_cp_dfset.add_argument("for", help="CP.x *.for file to be parsed.\n")
    parser_cp_dfset.add_argument(
        "-i",
        "--interval",
    ...
    parser_cp_wf.add_argument(
        "Filename", help="CP.x (cp-wf) stdout file to be parsed.\n"
    )
    ...
    parser_cpmd_energy = cpmd_sub_parsers.add_parser(
        "energy", help="cpmd.x ENERGIES parser"
    )
    parser_cpmd_energy.add_argument(
        "-F",
        "--Filename",
    ...
    parser_cpmd_force = cpmd_sub_parsers.add_parser(
        "force", help="cpmd.x FTRAJECTORY parser"
    )
    parser_cpmd_force.add_argument(
        "-F",
        "--Filename",
    ...
    parser_cpmd_dipole = cpmd_sub_parsers.add_parser(
        "dipole", help="cpmd.x DIPOLE parser"
    )
    parser_cpmd_dipole.add_argument(
        "-F", "--Filename", help="CPMD.x DIPOLE file to be parsed.\n", default="DIPOLE"
    )
    parser_cpmd_dipole.add_argument(
        "-s",
        "--stdout",
    ...
    parser_cpmd_dfset = cpmd_sub_parsers.add_parser(
        "dfset", help="cpmd.x to DFSET converter"
    )
    parser_cpmd_dfset.add_argument(
        "-F",
        "--Filename",
        help="CPMD.x ENERGIES file to be parsed.\n",
        default="ENERGIES",
    ...
    parser_cpmd_dfset.add_argument(
        "-s", "--start", help="start step to extract structures.\n", default=0
    )
    ...
    parser_cpmd_xyz.add_argument(
        "-o", "--output", help="resultant xyz filename.\n", default="IONS+CENTERS.xyz"
    )
    parser_cpmd_xyz.set_defaults(handler=cpextract_cpmd.command_cpmd_xyz)

    # cpextract cpmd sort
    parser_cpmd_sort = cpmd_sub_parsers.add_parser(
        "sort", help="cpmd.x xyz parser to sort"
    )
    parser_cpmd_sort.add_argument(
        "-i",
        "--input",
        help="CPMD.x IONS+CENTERS.xyz file to be parsed.\n",
        default="IONS+CENTERS.xyz",
    ...
    parser_cpmd_sort.add_argument(
        "-s", "--sortfile", help="sort file by CPmake.py.\n", default="sort_index.txt"
    )
    parser_cpmd_sort.set_defaults(handler=cpextract_cpmd.command_cpmd_xyzsort)

    # cpextract cpmd addlattice
    parser_cpmd_addlattice = cpmd_sub_parsers.add_parser(
        "addlattice", help="cpmd.x addlattice parser to sort"
    )
    parser_cpmd_addlattice.add_argument(
        "-i",
        "--input",
    ...
    # cpextract cpmd msd
    parser_cpmd_msd = cpmd_sub_parsers.add_parser(
        "msd", help="cpmd.x xyz parser to calculate msd"
    )
    parser_cpmd_msd.add_argument(
        "-F",
        "--Filename",
    ...
    parser_cpmd_vdos.add_argument(
        "-n", "--numatom", help="number of atoms in a molecule \n", required=True
    )
    parser_cpmd_vdos.add_argument(
        "--maxat", help="max atoms for total mode \n", default=None
    )
    parser_cpmd_vdos.add_argument(
        "-i",
        "--initial",
    ...
    # cpextract cpmd roo
    parser_cpmd_roo = cpmd_sub_parsers.add_parser(
        "roo",
    ...
    # cpextract cpmd roo
    parser_cpmd_distanceft = cpmd_sub_parsers.add_parser(
        "distanceft",
        help="cpmd.x xyz parser to calculate rOO correlation function",
        description="cpmd.x xyz parser to calculate oxygen-oxygen distance correlation function",
    )
    parser_cpmd_distanceft.add_argument(
        "-F",
        "--filename",
        help="CPMD.x xyz file to be parsed. It must include lattice information. \n",
        default="IONS+CENTERS.xyz",
    ...
    # cpextract cpmd oh
    parser_cpmd_oh = cpmd_sub_parsers.add_parser(
        "oh",
    ...
    parser_diel_histgram = diel_sub_parsers.add_parser(
        "histgram", help="post-process molecule_dipole.txt parser to plot histgram"
    )
    parser_diel_histgram.add_argument(
        "-F",
        "--Filename",
    ...
    parser_diel_histgram.add_argument(
        "-m", "--max", help="max value of histgram.\n", default=None
    )
    parser_diel_histgram.set_defaults(handler=histgram.command_diel_histgram)

    # CPextract.py diel total
    parser_diel_total = diel_sub_parsers.add_parser(
        "plot",
    ...
    parser_diel_total.add_argument(
        "-F",
        "--Filename",
    ...
    parser_diel_spectra.add_argument(
        "-F",
        "--Filename",
        help="filename of total_dipole.txt. Currently, only total_dipole.txt is supported.\n",
        default="total_dipole.txt",
    ...
    parser_diel_spectra.add_argument(
        "-s", "--start", help="start step. default is 0.\n", default="0"
    )
    parser_diel_spectra.add_argument(
        "-e",
        "--end",
    ...
    parser_diel_spectra.add_argument(
        "-f", "--fft", help="If use FFT for acf. default is True.\n", default="True"
    )
    ...
    parser_diel_dielconst = diel_sub_parsers.add_parser(
        "dielconst",
    ...
    parser_diel_dielconst.add_argument(
        "-F",
        "--Filename",
        help="filename of total_dipole.txt. Currently, only total_dipole.txt is supported.\n",
        default="total_dipole.txt",
    ...
    # CPextract.py diel const
    parser_diel_gfactor = diel_sub_parsers.add_parser(
        "gfactor",
    ...
    parser_diel_gfactor.add_argument(
        "-F",
        "--Filename",
        help="filename of total_dipole.txt. Currently, only total_dipole.txt is supported.\n",
        default="molecule_dipole.txt",
    ...
    parser_diel_mol = diel_sub_parsers.add_parser(
        "mol",
    ...
    parser_diel_resample.add_argument(
        "-F", "--Filename", help="filename of diel.csv.\n", required=True
    )
    parser_diel_resample.add_argument(
        "-n", "--num", help="The number of data to.\n", default="20000"
    )
    parser_diel_resample.set_defaults(
        handler=mlwc.cmdline.cpextract_diel.resample.command_diel_resample
    )

    # CPextract.py diel average
    parser_diel_average = diel_sub_parsers.add_parser(
        "average",
    ...
    parser_diel_average.add_argument(
        "-F", "--Filename", help="filename of diel.csv.\n", required=True
    )
    parser_diel_average.add_argument(
        "-w",
    ...
    parser_diel_average.add_argument(
        "-M", "--maxfreq", help="The maximum frequency in kayser.\n", default="4000"
    )
    parser_diel_average.set_defaults(
        handler=mlwc.cmdline.cpextract_diel.average.command_diel_average
    )
