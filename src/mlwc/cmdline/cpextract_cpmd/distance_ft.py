...
class distance_vector_autocorrelation:
    ...
    def calc_distanceft(self) -> np.ndarray:
        ...
        # OHボンドのO原子のリスト
        atom1_list: list[int] = [
            self._NUM_ATOM_PER_MOL * mol_id + self._index[0]
            for mol_id in range(self.NUM_MOL)
        ]
        atom2_list: list[int] = [
            self._NUM_ATOM_PER_MOL * mol_id + self._index[1]
            for mol_id in range(self.NUM_MOL)
        ]
        # calculate atomic distances
        hydrogen_bond_list: np.ndarray = mlwc.fourier.hydrogenbond.calc_distance(
            self._traj, atom1_list, atom2_list
        )
        np.save(
            self.__filename + f"_atomindex_{self._index[0]}_{self._index[1]}_list.npy",
            hydrogen_bond_list,
        )
    ...
    def calc_vectorft(self) -> np.ndarray:
        ...
        # calculate OH vector
        bond_vectors: np.array = mlwc.fourier.hydrogenbond.calc_oh(
            self._traj, atom1_list, atom2_list
        )
        np.save(
            self.__filename + f"_angle_{self._index[0]}_{self._index[1]}_list.npy",
            bond_vectors,
        )
        # 全ての時系列に対して自己相関を計算 (axis=1で各行に対して自己相関を計算)
        # 'same' モードで時系列の長さを維持
    ...
    def calc_angleft(self) -> np.ndarray:
        ...
        start_2_list: list[int] = [
            self._NUM_ATOM_PER_MOL * mol_id + self._index[2]
            for mol_id in range(self.NUM_MOL)
        ]
        end_2_list: list[int] = [
            self._NUM_ATOM_PER_MOL * mol_id + self._index[3]
            for mol_id in range(self.NUM_MOL)
        ]

        # calculate two (normalized) vectors
        bond_vectors_1: np.array = mlwc.fourier.hydrogenbond.calc_oh(
            self._traj, start_1_list, end_1_list
        )
        bond_vectors_2: np.array = mlwc.fourier.hydrogenbond.calc_oh(
        ...
        correlations = np.apply_along_axis(
            lambda x: scipy.signal.correlate(x, x, mode="full"),
            axis=0,
            arr=bond_vector_angle,
        )

        # 自己相関の平均化 (axis=1で全ての時系列に対する平均を取る)
        mean_correlation = np.mean(correlations, axis=1)[
            len(bond_vector_angle) - 1 :
        ]  # acf
        return mean_correlation
    ...
    def save_files(self, mean_correlation: np.ndarray, strategy: str = "distance"):
        ...
        to_csv_with_comment(
            df_acf,
            self.comment,
            self.__filename + f"_{strategy}_{self._index[0]}_{self._index[1]}_acf.csv",
        )
        ...
        to_csv_with_comment(
            df_roo,
            self.comment,
            self.__filename + f"_{strategy}_{self._index[0]}_{self._index[1]}_ft.csv",
        )
...
def command_cpmd_ft(args):  # distance, vector, angle correlation of two atoms
    ...
    0
    """
    roo = distance_vector_autocorrelation(
        args.filename,
        args.molfile,
        args.index,
        float(args.timestep),
