...
def mlpred(yaml_filename: str) -> None:
    ...
    model_co = load_model(
        input_predict.model_dir + "/model_co.pt", input_predict.device
    )
    model_oh = load_model(
        input_predict.model_dir + "/model_oh.pt", input_predict.device
    )
    model_cc = load_model(
        input_predict.model_dir + "/model_cc.pt", input_predict.device
    )
    model_o = load_model(input_predict.model_dir + "/model_o.pt", input_predict.device)
    # below is for coh/coc bindings
    model_coc = load_model(
        input_predict.model_dir + "/model_coc.pt", input_predict.device
    )
    model_coh = load_model(
        input_predict.model_dir + "/model_coh.pt", input_predict.device
    )
    ...
    for fr_index, fr_atoms in enumerate(atoms_traj):
        ...
        if len(itp_data.bond_index["CH_1_bond"]) != 0 and model_ch != None:
            ...
            y_pred_ch = (
                model_ch(X_ch.to(input_predict.device)).to("cpu").detach().numpy()
        ...
        if len(itp_data.bond_index["CO_1_bond"]) != 0 and model_co != None:
            ...
            y_pred_co = (
                model_co(X_co.to(input_predict.device)).to("cpu").detach().numpy()
            )
            list_wc_coords.append(
                (
                    bond_centers
        ...
        if len(itp_data.bond_index["CC_1_bond"]) != 0 and model_cc != None:
            ...
            y_pred_cc = (
                model_cc(X_cc.to(input_predict.device)).to("cpu").detach().numpy()
            )
            list_wc_coords.append(
                (
                    bond_centers
        ...
        if len(itp_data.bond_index["OH_1_bond"]) != 0 and model_oh != None:
            ...
            y_pred_oh = (
                model_oh(X_oh.to(input_predict.device)).to("cpu").detach().numpy()
            )
        ...
        if len(itp_data.coc_index) != 0 and model_coc != None:
            ...
            y_pred_coc = (
                model_coc(X_coc.to(input_predict.device)).to("cpu").detach().numpy()
            )
        ...
        if len(itp_data.coh_index) != 0 and model_coh != None:
            ...
            y_pred_coh = (
                model_coh(X_coh.to(input_predict.device)).to("cpu").detach().numpy()
            )
        ...
        molecule_dipole = (
            y_pred_ch.reshape((NUM_MOL, -1, 3)).sum(axis=1)
