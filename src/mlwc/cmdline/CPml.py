...
def make_merge_descs(len_traj: int, NUM_MOL: int, bond_index, savedir: str, name: str):
    ...
    if len(bond_index) != 0:
        merge_descs = np.empty(
            [len_traj, NUM_MOL * len(bond_index), 288]
        )  # TODO :: hard code
        ...
        np.save(
            f"merge_descs_{name}.npy",
            merge_descs.reshape([len_traj * NUM_MOL * len(bond_index), 288]),
        )  # TODO :: hard code
        np.save(
            f"merge_true_y_{name}.npy",
            merge_truey.reshape([len_traj * NUM_MOL * len(bond_index), 3]),
        )
...
def calc_descripter_frame_descmode1(
    atoms_fr, fr, savedir, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS, var_des
):
    """
    ...
    mol_with_BC = cpmd.asign_wcs.make_ase_with_BCs(
        atoms_fr.get_atomic_numbers(),
        NUM_MOL,
        UNITCELL_VECTORS,
        list_mol_coords,
        list_bond_centers,
    ...
    if len(itp_data.ring_bond_index) != 0:
        ...
        for bond_center in ring_cent_mol:
            ...
            i += 1
        np.savetxt(
            savedir + "Descs_ring_" + str(fr) + ".csv", Descs_ring, delimiter=","
        )
        del Descs_ring
...
def calc_molecule_dipole(
    list_mu_bonds, list_mu_pai, list_mu_lpO, list_mu_lpN, NUM_MOL: int
) -> np.array:
    """
    ...
    list_molecule_dipole = np.sum(list_mu_bonds, axis=1) + np.sum(list_mu_lpO, axis=1)
    return list_molecule_dipole


def calc_descripter_frame2(
    atoms_fr,
    wannier_fr,
    fr,
    savedir,
    ...
    mol_with_WC = cpmd.asign_wcs.make_ase_with_WCs(
        atoms_fr.get_atomic_numbers(),
        NUM_MOL,
        UNITCELL_VECTORS,
        list_mol_coords,
    ...
    if len(itp_data.bond_index["CH_1_bond"]) != 0:
        if not var_des.trueonly:
            ...
            if np.max(Descs_ch) > 5.0:  # 記述子が大きすぎる場合に警告
                print(
                    " WARNING :: Descs_ch is too large !! :: {}".format(
                        np.max(Descs_ch)
                    )
                )
            del Descs_ch
    ...
    if len(itp_data.bond_index["OH_1_bond"]) != 0:
        if not var_des.trueonly:
            ...
            if np.max(Descs_oh) > 5.0:  # 記述子が大きすぎる場合に警告
                print(
                    " WARNING :: Descs_oh is too large !! :: {}".format(
                        np.max(Descs_oh)
                    )
                )
            del Descs_oh
    ...
    if len(itp_data.bond_index["CO_1_bond"]) != 0:
        if not var_des.trueonly:
            ...
            if np.max(Descs_co) > 5.0:
                print(
                    " WARNING :: Descs_co is too large !! :: {}".format(
                        np.max(Descs_co)
                    )
                )
            del Descs_co
    ...
    if len(itp_data.bond_index["CC_1_bond"]) != 0:
        if not var_des.trueonly:
            ...
            if np.max(Descs_cc) > 5.0:
                print(
                    " WARNING :: Descs_cc is too large !! :: {}".format(
                        np.max(Descs_cc)
                    )
                )
            del Descs_cc
        ...
        del True_y_cc
    if (
        len(itp_data.o_list) != 0
    ):  # !! 2023/10/08 calc_lonepair_desc**のinputの8はなくても大丈夫になってる
        if not var_des.trueonly:
            Descs_o = DESC.calc_lonepair_descripter_at_frame(
                atoms_fr,
                list_mol_coords,
            ...
            if np.max(Descs_o) > 5.0:
                print(
                    " WARNING :: Descs_o is too large !! :: {}".format(np.max(Descs_o))
                )
            del Descs_o
    ...
    if len(itp_data.coc_index) != 0:
        ...
        np.savetxt(
            savedir + "True_y_coc_" + str(fr) + ".csv",
            True_y_coc.reshape(-1, 3),
            delimiter=",",
        )
    ...
    if len(itp_data.coh_index) != 0:
        ...
        if not var_des.trueonly:
            ...
            np.savetxt(
                savedir + "Descs_coh_" + str(fr) + ".csv", Descs_coh, delimiter=","
            )
        ...
        np.savetxt(
            savedir + "True_y_coh_" + str(fr) + ".csv",
            True_y_coh.reshape(-1, 3),
            delimiter=",",
        )
...
def calc_descripter_frame3(
    ...
    for i in range(NUM_MOL):
        Mtot.append(
            np.sum(list_mu_bonds[i], axis=0)
            + np.sum(list_mu_pai[i], axis=0)
            + np.sum(list_mu_lpO[i], axis=0)
            + np.sum(list_mu_lpN[i], axis=0)
        )
    Mtot = np.array(Mtot)
    # unit cellの全双極子モーメントの計算
    total_dipole = np.sum(Mtot, axis=0)
...
def calc_descripter_frame_and_predict_dipole(
    atoms_fr,
    fr,
    itp_data,
    NUM_MOL,
    ...
    if model_ch_2 != None:
        y_pred_ch = (
            model_ch_2(X_ch.reshape(-1, nfeatures).to(device))
            .to("cpu")
            .detach()
            .numpy()
        )
    if model_co_2 != None:
        y_pred_co = (
            model_co_2(X_co.reshape(-1, nfeatures).to(device))
            .to("cpu")
            .detach()
            .numpy()
        )
    if model_oh_2 != None:
        y_pred_oh = (
            model_oh_2(X_oh.reshape(-1, nfeatures).to(device))
            .to("cpu")
            .detach()
            .numpy()
        )
    if model_cc_2 != None:
        y_pred_cc = (
            model_cc_2(X_cc.reshape(-1, nfeatures).to(device))
            .to("cpu")
            .detach()
            .numpy()
        )
    if model_o_2 != None:
        y_pred_o = (
            model_o_2(X_o.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
        )
    ...
    # 予測したモデルを使ったUnit Cellの双極子モーメントの計算
    sum_dipole = (
        np.sum(y_pred_ch, axis=0)
        + np.sum(y_pred_oh, axis=0)
        + np.sum(y_pred_co, axis=0)
        + np.sum(y_pred_o, axis=0)
    )  # +np.sum(y_pred_cc,axis=0)
...
def main():
    ...
    if if_calc_descripter:  # descripter計算をする場合，trajectoryを読み込む
        ...
        print(
            " We recommend to use less than 5GB. because reading too large file consumes too much memory."
        )
        ...
        if int(var_des.haswannier) == True:
            ...
            time_end = time.time()
            print(
                " Finish reading trajectory via cpmd.read_traj_cpmd.raw_xyz_divide_aseatoms_list. Time is {} sec.".format(
                    time_end - time_start
                )
            )
            ...
            time_end = time.time()
            print(
                " Finish reading trajectory via ase.io.read. Time is {} sec.".format(
                    time_end - time_start
                )
            )
            ...
            traj = traj[
                :: var_des.interval
            ]  # trajの場合もintervalを設定する．（動くか不明）
            time_end = time.time()
            print(
                " Finish reading trajectory via ase.io.trajectory. Time is {} sec.".format(
                    time_end - time_start
                )
            )
        ...
        # * traj変数の大きさを出力
        print(
            " Size of variable traj is ... {} KB. ".format(sys.getsizeof(traj) / 1000)
        )
    ...
    if (
        if_calc_descripter and not if_calc_predict
    ):  # descripter計算のみの場合，記述子を保存して終了
        # *
        ...
        if var_des.descmode == "1":
            ...
            result_ase = joblib.Parallel(n_jobs=-1, verbose=50)(
                joblib.delayed(calc_descripter_frame_descmode1)(
                    atoms_fr,
                    fr,
                    var_des.savedir,
                    itp_data,
        ...
        if var_des.descmode == "2":
            ...
            result = joblib.Parallel(n_jobs=-1, verbose=50)(
                joblib.delayed(calc_descripter_frame2)(
                    atoms_fr,
                    wannier_fr,
                    fr,
                    var_des.savedir,
                ...
                for fr, (atoms_fr, wannier_fr) in enumerate(zip(traj, wannier_list))
            )
            ...
            # 分子の双極子を保存
            np.save(
                var_des.savedir + "/molecule_dipole.npy",
            ...
            make_merge_descs(
                len(traj), NUM_MOL, itp_data.coc_index, var_des.savedir, "coc"
            )
            make_merge_descs(
                len(traj), NUM_MOL, itp_data.coh_index, var_des.savedir, "coh"
            )
        ...
        if var_des.descmode == "3":
            ...
            # result = joblib.Parallel(n_jobs=-1, verbose=50)(joblib.delayed(calc_descripter_frame)(atoms_fr,wannier_fr,fr,var_des.savedir) for fr,(atoms_fr, wannier_fr) in enumerate(zip(traj,wannier_list)))
            result = joblib.Parallel(n_jobs=-1, verbose=50)(
                joblib.delayed(calc_descripter_frame3)(
                    atoms_fr,
                    wannier_fr,
                    fr,
                    var_des.savedir,
    ...
    if if_calc_predict:
        ...
        if os.path.isfile(var_pre.model_dir + "model_ch_weight4.pth"):
            model_ch.load_state_dict(
                torch.load(var_pre.model_dir + "model_ch_weight4.pth")
            )
        ...
        if os.path.isfile(var_pre.model_dir + "model_co_weight4.pth"):
            model_co.load_state_dict(
                torch.load(var_pre.model_dir + "model_co_weight4.pth")
            )
        ...
        if os.path.isfile(var_pre.model_dir + "model_oh_weight4.pth"):
            model_oh.load_state_dict(
                torch.load(var_pre.model_dir + "model_oh_weight4.pth")
            )
        ...
        if os.path.isfile(var_pre.model_dir + "model_cc_weight4.pth"):
            model_cc.load_state_dict(
                torch.load(var_pre.model_dir + "model_cc_weight4.pth")
            )
        ...
        if os.path.isfile(var_pre.model_dir + "model_o_weight4.pth"):
            model_o.load_state_dict(
                torch.load(var_pre.model_dir + "model_o_weight4.pth")
            )
        ...
        if os.path.isfile(var_pre.model_dir + "model_coc_weight4.pth"):
            model_coc.load_state_dict(
                torch.load(var_pre.model_dir + "model_coc_weight4.pth")
            )
        ...
        if os.path.isfile(var_pre.model_dir + "model_coh_weight4.pth"):
            model_coh.load_state_dict(
                torch.load(var_pre.model_dir + "model_coh_weight4.pth")
            )
    ...
    if not if_calc_descripter and if_calc_predict:
        ...
        def predict_dipole_mode1(fr, desc_dir):
            ...
            if os.path.isfile(desc_dir + "Descs_o_" + str(fr) + ".csv"):
                ...
                X_o = torch.from_numpy(descs_X_o.astype(np.float32)).clone()
                y_pred_o = (
                    model_o_2(X_o.reshape(-1, nfeatures).to(device))
                    .to("cpu")
                    .detach()
                    .numpy()
                )
            ...
            # !! >>>> ここからCOH/COC >>>
            if (
                os.path.isfile(desc_dir + "Descs_coc_" + str(fr) + ".npy")
                and model_coc_2 != None
            ):
                descs_X_coc = np.load(desc_dir + "Descs_coc_" + str(fr) + ".npy")
                X_coc = torch.from_numpy(
                    descs_X_coc.astype(np.float32)
                ).clone()  # オリジナルの記述子を一旦tensorへ
                y_pred_coc = (
                    model_coc_2(X_coc.reshape(-1, nfeatures).to(device))
                    .to("cpu")
                    .detach()
                    .numpy()
                )
                ...
                if var_pre.save_truey:
                    np.save(
                        var_pre.desc_dir + "y_pred_coc_" + str(fr) + ".npy", y_pred_coc
                    )
            if (
                os.path.isfile(desc_dir + "Descs_coh_" + str(fr) + ".npy")
                and model_coh_2 != None
            ):
                descs_X_coh = np.load(desc_dir + "Descs_coh_" + str(fr) + ".npy")
                X_coh = torch.from_numpy(
                    descs_X_coh.astype(np.float32)
                ).clone()  # オリジナルの記述子を一旦tensorへ
                y_pred_coh = (
                    model_coh_2(X_coh.reshape(-1, nfeatures).to(device))
                    .to("cpu")
                    .detach()
                    .numpy()
                )
                ...
                if var_pre.save_truey:
                    np.save(
                        var_pre.desc_dir + "y_pred_coh_" + str(fr) + ".npy", y_pred_coh
                    )
            # !! <<< ここまでCOH/COC <<<
            # print("DEBUG :: shape ch/co/oh/o :: {0} {1} {2} {3}".format(np.shape(y_pred_ch),np.shape(y_pred_co),np.shape(y_pred_oh),np.shape(y_pred_o)))
            # if fr == 0:
        ...
        def predict_dipole(fr, desc_dir):
            ...
            y_pred_o = y_pred_o.reshape((-1, 3))
            print(
                "DEBUG :: shape ch/co/oh/o :: {0} {1} {2} {3}".format(
                    np.shape(y_pred_ch),
                    np.shape(y_pred_co),
                    np.shape(y_pred_oh),
                    np.shape(y_pred_o),
        ...
        result_dipole = joblib.Parallel(n_jobs=-1, verbose=50)(
            joblib.delayed(predict_dipole_mode1)(fr, var_pre.desc_dir)
            for fr in range(num_structure)
        )
        import numpy as np
    ...
    if if_calc_descripter and if_calc_predict and var_des.desc_coh:
        ...
        print(" ------------------- ")
        print(
            " This is COH/COC case (if_calc_descripter and if_calc_predict and var_des.desc_coh) "
        )
        print(" ------------------- ")
    ...
    if if_calc_descripter and if_calc_predict:
        ...
        if var_des.descmode == "1":
            ...
            def calc_descripter_frame_and_predict_dipole(
                ...
                if len(itp_data.ring_bond_index) != 0:
                    ...
                    for bond_center in ring_cent_mol:
                        mol_id = i % NUM_MOL // 1
                        Descs_ring.append(
                            DESC.get_desc_bondcent(atoms_fr, bond_center, mol_id)
                        )
                        i += 1
            ...
            if not __debug__:
                ...
                for i in range(ave):
                    ...
                    print(
                        "len(traj[i*cpu_size:(i+1)*cpu_size]) :: {}".format(
                            len(traj[i * cpu_size : (i + 1) * cpu_size])
                        )
                    )
                    # trajをcpu_sizeだけ読んでjoblibに渡す
                    for fr, atoms_fr in enumerate(
                        traj[i * cpu_size : (i + 1) * cpu_size]
                    ):
            ...
            if (
                sys.getsizeof(traj) / 1000 > 0
            ):  # < 100: # 100KB以下の場合は通常のjoblibを使う．
                print(
                ...
                result = joblib.Parallel(n_jobs=OMP_NUM_THREADS, verbose=50)(
                    joblib.delayed(calc_descripter_frame_and_predict_dipole)(
                        atoms_fr, fr, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS
                    )
                    for fr, atoms_fr in enumerate(traj)
                )
                ...
                cpu_size = (
                    OMP_NUM_THREADS * 100
                )  # cpu_countではなく，OMP_NUM_THREADSを使う．
                ave, res = divmod(len(traj), cpu_size)
                print("ave :: {}, res :: {}".format(ave, res))
                for i in range(ave):
                    print(
                    ...
                    result_dipole = joblib.Parallel(n_jobs=-1, verbose=50)(
                        ...
                        for fr, atoms_fr in enumerate(tmp_traj)
                    )
                    ...
                    # np.save(var_des.savedir+"/wannier_dipole.npy", result_dipole)
                    np.save(
                        var_des.savedir + "/result_dipole_" + str(i) + ".npy",
                        result_dipole,
                    )
                    print(" finish save step :: {}".format(i))
                # 最後のあまりの部分
                ...
                result_dipole = joblib.Parallel(n_jobs=-1, verbose=50)(
                    ...
                    for fr, atoms_fr in enumerate(traj[ave * cpu_size :])
                )
                print(
                    " np.shape(result_dipole) == ave ?? :: {}".format(
        ...
        if var_des.descmode == "2":
            ...
            def calc_descripter_frame(
                ...
                if len(itp_data.bond_index["CH_1_bond"]) != 0 and model_ch_2 != None:
                    ...
                    if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                        # 予測値の保存
                        np.save(
                            var_pre.desc_dir + "/y_pred_ch_" + str(fr) + ".npy",
                            y_pred_ch,
                        )
                if len(itp_data.bond_index["CO_1_bond"]) != 0 and model_co_2 != None:
                    ...
                    if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                        # 予測値の保存
                        np.save(
                            var_pre.desc_dir + "/y_pred_co_" + str(fr) + ".npy",
                            y_pred_co,
                        )
                if len(itp_data.bond_index["OH_1_bond"]) != 0 and model_oh_2 != None:
                    ...
                    if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                        # 予測値の保存
                        np.save(
                            var_pre.desc_dir + "/y_pred_oh_" + str(fr) + ".npy",
                            y_pred_oh,
                        )
                if len(itp_data.bond_index["CC_1_bond"]) != 0 and model_cc_2 != None:
                    ...
                    if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                        # 予測値の保存
                        np.save(
                            var_pre.desc_dir + "/y_pred_cc_" + str(fr) + ".npy",
                            y_pred_cc,
                        )
                if len(itp_data.o_list) != 0 and model_o_2 != None:
                    ...
                    if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                        # 予測値の保存
                        np.save(
                            var_pre.desc_dir + "/y_pred_o_" + str(fr) + ".npy", y_pred_o
                        )
                # !! >>>> ここからCOH/COC >>>
                ...
                if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                    ...
                    if var_des.desc_coh:
                        np.save(
                            var_pre.desc_dir + "y_pred_coc_" + str(fr) + ".npy",
                            y_pred_coc,
                        )
                        np.save(
                            var_pre.desc_dir + "y_pred_coh_" + str(fr) + ".npy",
                            y_pred_coh,
                        )
                    # !! <<< ここまでCOH/COC <<<
                    # 真値の保存
                    ...
                    np.save(
                        var_pre.desc_dir + "/y_true_ch_" + str(fr) + ".npy", True_y_ch
                    )
                    np.save(
                        var_pre.desc_dir + "/y_true_co_" + str(fr) + ".npy", True_y_co
                    )
                    np.save(
                        var_pre.desc_dir + "/y_true_oh_" + str(fr) + ".npy", True_y_oh
                    )
                    np.save(
                        var_pre.desc_dir + "/y_true_cc_" + str(fr) + ".npy", True_y_cc
                    )
                    np.save(
                        var_pre.desc_dir + "/y_true_o_" + str(fr) + ".npy", True_y_o
                    )
            ...
            if var_des.step != None:  # stepが決まっている場合はこちらで設定してしまう．
                print("STEP is manually set :: {}".format(var_des.step))
                traj = traj[: var_des.step]
            result_dipoles = joblib.Parallel(n_jobs=-1, verbose=50)(
                joblib.delayed(calc_descripter_frame)(
                    atoms_fr,
                    wannier_fr,
                    fr,
                    itp_data,
