...
def make_merge_descs(len_traj: int, NUM_MOL: int, bond_index, savedir: str, name: str):
    ...
    import os
    if len(bond_index) != 0:
        merge_descs = np.empty(
            [len_traj, NUM_MOL*len(bond_index), 288])  # TODO :: hard code
        merge_truey = np.empty([len_traj, NUM_MOL*len(bond_index), 3])
        for i in range(len_traj):
            if i % 1000 == 0:
                print(f"reading steps = {i}")
            # read descriptor
            tmp_descs = np.loadtxt(
                savedir+'/Descs_'+name+'_'+str(i)+'.csv', delimiter=',')
            tmp_truey = np.loadtxt(
                savedir+'/True_y_'+name+'_'+str(i)+'.csv', delimiter=',')
            merge_descs[i] = tmp_descs
            merge_truey[i] = tmp_truey
        np.save(f"merge_descs_{name}.npy",  merge_descs.reshape(
            [len_traj*NUM_MOL*len(bond_index), 288]))  # TODO :: hard code
        np.save(f"merge_true_y_{name}.npy", merge_truey.reshape(
            [len_traj*NUM_MOL*len(bond_index), 3]))
...
def calc_descripter_frame_descmode1(atoms_fr, fr, savedir, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS, var_des):
    ...
    results = ASIGN.aseatom_to_mol_coord_bc(
        atoms_fr, itp_data, itp_data.bonds_list)
    list_mol_coords, list_bond_centers = results
    # BCをアサインしたase.atomsを作成する
    mol_with_BC = cpmd.asign_wcs.make_ase_with_BCs(atoms_fr.get_atomic_numbers(
    ), NUM_MOL, UNITCELL_VECTORS, list_mol_coords, list_bond_centers)
    ...
    if len(itp_data.ring_bond_index) != 0:
        ...
        ring_cent_mol = cpmd.descripter.find_specific_ringcenter(
            list_bond_centers, itp_data.ring_bond_index, 8, NUM_MOL)
        i = 0
        for bond_center in ring_cent_mol:
            ...
            i += 1
        np.savetxt(savedir+'Descs_ring_'+str(fr) +
    ...
    if len(itp_data.bond_index['CH_1_bond']) != 0:
        Descs_ch = DESC.calc_bond_descripter_at_frame(
            atoms_fr, list_bond_centers, itp_data.bond_index['CH_1_bond'], var_des.desctype)
    ...
    if len(itp_data.bond_index['CC_1_bond']) != 0:
        Descs_cc = DESC.calc_bond_descripter_at_frame(
            atoms_fr, list_bond_centers, itp_data.bond_index['CC_1_bond'], var_des.desctype)
    ...
    if len(itp_data.bond_index['CO_1_bond']) != 0:
        Descs_co = DESC.calc_bond_descripter_at_frame(
            atoms_fr, list_bond_centers, itp_data.bond_index['CO_1_bond'], var_des.desctype)
    ...
    if len(itp_data.bond_index['OH_1_bond']) != 0:
        Descs_oh = DESC.calc_bond_descripter_at_frame(
            atoms_fr, list_bond_centers, itp_data.bond_index['OH_1_bond'], var_des.desctype)
    ...
    if len(itp_data.o_list) != 0:
        Descs_o = DESC.calc_lonepair_descripter_at_frame(
            atoms_fr, list_mol_coords, itp_data.o_list, 8, var_des.desctype)
...
def calc_molecule_dipole(list_mu_bonds, list_mu_pai, list_mu_lpO, list_mu_lpN, NUM_MOL: int) -> np.array:
    ...
    # +np.sum(list_mu_pai,axis=1)+np.sum(list_mu_lpN,axis=1)
    list_molecule_dipole = np.sum(
        list_mu_bonds, axis=1)+np.sum(list_mu_lpO, axis=1)
...
def calc_descripter_frame2(atoms_fr, wannier_fr, fr, savedir, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS, double_bonds, var_des):
    ...
    results_mu = ASIGN.calc_mu_bond_lonepair(
        wannier_fr, atoms_fr, itp_data.bonds_list, itp_data, double_bonds)
    ...
    mol_with_WC = cpmd.asign_wcs.make_ase_with_WCs(atoms_fr.get_atomic_numbers(
    ), NUM_MOL, UNITCELL_VECTORS, list_mol_coords, list_bond_centers, list_bond_wfcs, list_pi_wfcs, list_lpO_wfcs, list_lpN_wfcs)
    ...
    # 分子双極子の計算
    list_molecule_dipole = calc_molecule_dipole(
        list_mu_bonds, list_mu_pai, list_mu_lpO, list_mu_lpN, NUM_MOL)
    ...
    if len(itp_data.bond_index['CH_1_bond']) != 0:
        if not var_des.trueonly:
            Descs_ch = DESC.calc_bond_descripter_at_frame(
                atoms_fr, list_bond_centers, itp_data.bond_index['CH_1_bond'], var_des.desctype, var_des.Rcs, var_des.Rc, var_des.MaxAt)
            np.savetxt(savedir+'Descs_ch_'+str(fr) +
                       '.csv', Descs_ch, delimiter=',')
            if np.max(Descs_ch) > 5.0:  # 記述子が大きすぎる場合に警告
                print(" WARNING :: Descs_ch is too large !! :: {}".format(
                    np.max(Descs_ch)))
            del Descs_ch
        True_y_ch = DESC.calc_bondmu_descripter_at_frame(
            list_mu_bonds, itp_data.bond_index['CH_1_bond'])
        np.savetxt(savedir+'True_y_ch_'+str(fr) +
                   '.csv', True_y_ch, delimiter=',')
        del True_y_ch
    if len(itp_data.bond_index['OH_1_bond']) != 0:
        if not var_des.trueonly:
            Descs_oh = DESC.calc_bond_descripter_at_frame(
                atoms_fr, list_bond_centers, itp_data.bond_index['OH_1_bond'], var_des.desctype, var_des.Rcs, var_des.Rc, var_des.MaxAt)
            np.savetxt(savedir+'Descs_oh_'+str(fr) +
                       '.csv', Descs_oh, delimiter=',')
            if np.max(Descs_oh) > 5.0:  # 記述子が大きすぎる場合に警告
                print(" WARNING :: Descs_oh is too large !! :: {}".format(
                    np.max(Descs_oh)))
            del Descs_oh
        True_y_oh = DESC.calc_bondmu_descripter_at_frame(
            list_mu_bonds, itp_data.bond_index['OH_1_bond'])
        np.savetxt(savedir+'True_y_oh_'+str(fr) +
                   '.csv', True_y_oh, delimiter=',')
        del True_y_oh
    if len(itp_data.bond_index['CO_1_bond']) != 0:
        if not var_des.trueonly:
            Descs_co = DESC.calc_bond_descripter_at_frame(
                atoms_fr, list_bond_centers, itp_data.bond_index['CO_1_bond'], var_des.desctype, var_des.Rcs, var_des.Rc, var_des.MaxAt)
            np.savetxt(savedir+'Descs_co_'+str(fr) +
                       '.csv', Descs_co, delimiter=',')
            if np.max(Descs_co) > 5.0:
                print(" WARNING :: Descs_co is too large !! :: {}".format(
                    np.max(Descs_co)))
            del Descs_co
        True_y_co = DESC.calc_bondmu_descripter_at_frame(
            list_mu_bonds, itp_data.bond_index['CO_1_bond'])
        np.savetxt(savedir+'True_y_co_'+str(fr) +
                   '.csv', True_y_co, delimiter=',')
        del True_y_co
    if len(itp_data.bond_index['CC_1_bond']) != 0:
        if not var_des.trueonly:
            Descs_cc = DESC.calc_bond_descripter_at_frame(
                atoms_fr, list_bond_centers, itp_data.bond_index['CC_1_bond'], var_des.desctype, var_des.Rcs, var_des.Rc, var_des.MaxAt)
            np.savetxt(savedir+'Descs_cc_'+str(fr) +
                       '.csv', Descs_cc, delimiter=',')
            if np.max(Descs_cc) > 5.0:
                print(" WARNING :: Descs_cc is too large !! :: {}".format(
                    np.max(Descs_cc)))
            del Descs_cc
        True_y_cc = DESC.calc_bondmu_descripter_at_frame(
            list_mu_bonds, itp_data.bond_index['CC_1_bond'])
        np.savetxt(savedir+'True_y_cc_'+str(fr) +
                   '.csv', True_y_cc, delimiter=',')
        del True_y_cc
    if len(itp_data.o_list) != 0:  # !! 2023/10/08 calc_lonepair_desc**のinputの8はなくても大丈夫になってる
        if not var_des.trueonly:
            Descs_o = DESC.calc_lonepair_descripter_at_frame(
                atoms_fr, list_mol_coords, itp_data.o_list, 8, var_des.desctype, var_des.Rcs, var_des.Rc, var_des.MaxAt)
            np.savetxt(savedir+'Descs_o_'+str(fr) +
                       '.csv', Descs_o, delimiter=',')
            if np.max(Descs_o) > 5.0:
                print(" WARNING :: Descs_o is too large !! :: {}".format(
                    np.max(Descs_o)))
            del Descs_o
    ...
    if len(itp_data.coc_index) != 0:
        ...
        True_y_coc = DESC.calc_coc_bondmu_descripter_at_frame(
            list_mu_bonds, list_mu_lpO, itp_data.coc_index, itp_data.bond_index['CO_1_bond'])
        np.savetxt(savedir+'True_y_coc_'+str(fr)+'.csv',
                   True_y_coc.reshape(-1, 3), delimiter=',')
    ...
    if len(itp_data.coh_index) != 0:
        ...
        if not var_des.trueonly:
            ...
            Descs_coh = DESC.calc_lonepair_descripter_at_frame(
                atoms_fr, list_mol_coords, itp_data.o_list, 8, var_des.desctype)
            np.savetxt(savedir+'Descs_coh_'+str(fr) +
                       '.csv', Descs_coh, delimiter=',')
        True_y_coh = DESC.calc_coh_bondmu_descripter_at_frame(
            list_mu_bonds, list_mu_lpO, itp_data.coh_index, itp_data.bond_index['CO_1_bond'], itp_data.bond_index['OH_1_bond'])
        np.savetxt(savedir+'True_y_coh_'+str(fr)+'.csv',
                   True_y_coh.reshape(-1, 3), delimiter=',')
...
def calc_descripter_frame3(atoms_fr, wannier_fr, fr, savedir, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS, double_bonds, var_des):
    ...
    Mtot = []
    for i in range(NUM_MOL):
        Mtot.append(np.sum(list_mu_bonds[i], axis=0)+np.sum(list_mu_pai[i], axis=0)+np.sum(
            list_mu_lpO[i], axis=0)+np.sum(list_mu_lpN[i], axis=0))
    Mtot = np.array(Mtot)
    # unit cellの全双極子モーメントの計算
    total_dipole = np.sum(Mtot, axis=0)
    # 分子双極子の計算
...
def calc_descripter_frame_and_predict_dipole(atoms_fr, fr, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS, model_ch_2, model_oh_2, model_cc_2, model_co_2, model_o_2):
    ...
    # ch,oh,co,cc,
    Descs_ch = DESC.calc_bond_descripter_at_frame(
        atoms_fr, list_bond_centers, itp_data.bond_index['CH_1_bond'])
    Descs_oh = DESC.calc_bond_descripter_at_frame(
        atoms_fr, list_bond_centers, itp_data.bond_index['OH_1_bond'])
    Descs_co = DESC.calc_bond_descripter_at_frame(
        atoms_fr, list_bond_centers, itp_data.bond_index['CO_1_bond'])
    Descs_cc = DESC.calc_bond_descripter_at_frame(
        atoms_fr, list_bond_centers, itp_data.bond_index['CC_1_bond'])
    # oローンペア
    Descs_o = DESC.calc_lonepair_descripter_at_frame(
        atoms_fr, list_mol_coords, itp_data.o_list, 8)
    ...
    # 予測
    if model_ch_2 != None:
        y_pred_ch = model_ch_2(
            X_ch.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
    if model_co_2 != None:
        y_pred_co = model_co_2(
            X_co.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
    if model_oh_2 != None:
        y_pred_oh = model_oh_2(
            X_oh.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
    if model_cc_2 != None:
        y_pred_cc = model_cc_2(
            X_cc.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
    if model_o_2 != None:
        y_pred_o = model_o_2(
            X_o.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
    ...
    if fr == 0:  # デバッグ用
        ...
        print("y_pred_o  ::", y_pred_o)
    # 予測したモデルを使ったUnit Cellの双極子モーメントの計算
    sum_dipole = np.sum(y_pred_ch, axis=0)+np.sum(y_pred_oh, axis=0)+np.sum(
        y_pred_co, axis=0)+np.sum(y_pred_o, axis=0)  # +np.sum(y_pred_cc,axis=0)
...
def main():
    ...
    input_general, input_descripter, input_predict = ml.parse.locate_tag(
        inputs_list)
    ...
    if if_calc_descripter:  # descripter計算をする場合，trajectoryを読み込む
        ...
        if int(var_des.haswannier) == True:
            ...
            time_start = time.time()
            traj = ase.io.read(var_des.directory+var_des.xyzfilename,
                               index=slice(0, None, var_des.interval))
            time_end = time.time()
            print(" Finish reading trajectory via ase.io.read. Time is {} sec.".format(
                time_end-time_start))
            ...
            time_end = time.time()
            print(" Finish reading trajectory via ase.io.trajectory. Time is {} sec.".format(
                time_end-time_start))

        # * traj変数の大きさを出力
        print(" Size of variable traj is ... {} KB. ".format(
            sys.getsizeof(traj)/1000))
        ...
        import cpmd.descripter
        DESC = cpmd.descripter.descripter(
            NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS)
    ...
    if if_calc_descripter and not if_calc_predict:  # descripter計算のみの場合，記述子を保存して終了
        ...
        if var_des.descmode == "1":
            ...
            result_ase = joblib.Parallel(n_jobs=-1, verbose=50)(joblib.delayed(calc_descripter_frame_descmode1)(
                atoms_fr, fr, var_des.savedir, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS, var_des) for fr, atoms_fr in enumerate(traj))
        ...
        if var_des.descmode == "2":
            ...
            result = joblib.Parallel(n_jobs=-1, verbose=50)(joblib.delayed(calc_descripter_frame2)(atoms_fr, wannier_fr, fr, var_des.savedir, itp_data,
                                                                                                   NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS, double_bonds, var_des) for fr, (atoms_fr, wannier_fr) in enumerate(zip(traj, wannier_list)))
            ...
            # 双極子を保存
            np.save(var_des.savedir+"/wannier_dipole.npy",
                    np.array(result_dipole))
            # 分子の双極子を保存
            np.save(var_des.savedir+"/molecule_dipole.npy",
                    np.array(result_molecule_dipole))
            ...
            # 記述子をまとめる&古いものを削除
            make_merge_descs(
                len(traj), NUM_MOL, itp_data.bond_index['CH_1_bond'], var_des.savedir, "ch")
            make_merge_descs(
                len(traj), NUM_MOL, itp_data.bond_index['CC_1_bond'], var_des.savedir, "cc")
            make_merge_descs(
                len(traj), NUM_MOL, itp_data.bond_index['CO_1_bond'], var_des.savedir, "co")
            make_merge_descs(
                len(traj), NUM_MOL, itp_data.bond_index['OH_1_bond'], var_des.savedir, "oh")
            make_merge_descs(len(traj), NUM_MOL,
                             itp_data.o_list,        var_des.savedir, "o")
            make_merge_descs(len(traj), NUM_MOL,
                             itp_data.coc_index,     var_des.savedir, "coc")
            make_merge_descs(len(traj), NUM_MOL,
                             itp_data.coh_index,     var_des.savedir, "coh")
        ...
        if var_des.descmode == "3":
            ...
            if var_des.step != None:  # stepが決まっている場合はこちらで設定してしまう．
                ...
                traj = traj[:var_des.step]
            # result = joblib.Parallel(n_jobs=-1, verbose=50)(joblib.delayed(calc_descripter_frame)(atoms_fr,wannier_fr,fr,var_des.savedir) for fr,(atoms_fr, wannier_fr) in enumerate(zip(traj,wannier_list)))
            result = joblib.Parallel(n_jobs=-1, verbose=50)(joblib.delayed(calc_descripter_frame3)(atoms_fr, wannier_fr, fr, var_des.savedir, itp_data,
                                                                                                   NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS, double_bonds, var_des) for fr, (atoms_fr, wannier_fr) in enumerate(zip(traj, wannier_list)))
            ...
            make_merge_truey(
                len(traj), NUM_MOL, itp_data.bond_index['OH_1_bond'], var_des.savedir, "oh")
            make_merge_truey(len(traj), NUM_MOL,
                             itp_data.o_list,        var_des.savedir, "o")
            make_merge_truey(len(traj), NUM_MOL,
                             itp_data.coc_index,     var_des.savedir, "coc")
            make_merge_truey(len(traj), NUM_MOL,
    ...
    if if_calc_predict:
        ...
        if var_pre.modelmode == "rotate2":
            ...
            from mlwc.ml.mlmodel import NET_custom
            # # モデル（NeuralNetworkクラス）のインスタンス化
        ...
        if os.path.isfile(var_pre.model_dir+'model_ch_weight4.pth'):
            model_ch.load_state_dict(torch.load(
                var_pre.model_dir+'model_ch_weight4.pth'))
        ...
        if os.path.isfile(var_pre.model_dir+'model_co_weight4.pth'):
            model_co.load_state_dict(torch.load(
                var_pre.model_dir+'model_co_weight4.pth'))
        ...
        if os.path.isfile(var_pre.model_dir+'model_oh_weight4.pth'):
            model_oh.load_state_dict(torch.load(
                var_pre.model_dir+'model_oh_weight4.pth'))
        ...
        if os.path.isfile(var_pre.model_dir+'model_cc_weight4.pth'):
            model_cc.load_state_dict(torch.load(
                var_pre.model_dir+'model_cc_weight4.pth'))
        ...
        if os.path.isfile(var_pre.model_dir+'model_o_weight4.pth'):
            model_o.load_state_dict(torch.load(
                var_pre.model_dir+'model_o_weight4.pth'))
        ...
        if os.path.isfile(var_pre.model_dir+'model_coc_weight4.pth'):
            model_coc.load_state_dict(torch.load(
                var_pre.model_dir+'model_coc_weight4.pth'))
        ...
        if os.path.isfile(var_pre.model_dir+'model_coh_weight4.pth'):
            model_coh.load_state_dict(torch.load(
                var_pre.model_dir+'model_coh_weight4.pth'))
    ...
    if not if_calc_descripter and if_calc_predict:
        ...
        def predict_dipole_mode1(fr, desc_dir):
            ...
            if os.path.isfile(desc_dir+'Descs_ch_'+str(fr)+'.csv'):
                flag_ch = True
                descs_X_ch = np.loadtxt(
                    desc_dir+'Descs_ch_'+str(fr)+'.csv', delimiter=',')
                X_ch = torch.from_numpy(descs_X_ch.astype(
                    np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
                y_pred_ch = model_ch_2(
                    X_ch.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()   # 予測
            ...
            if os.path.isfile(desc_dir+'Descs_co_'+str(fr)+'.csv'):
                flag_co = True
                descs_X_co = np.loadtxt(
                    desc_dir+'Descs_co_'+str(fr)+'.csv', delimiter=',')
                X_co = torch.from_numpy(descs_X_co.astype(np.float32)).clone()
                y_pred_co = model_co_2(
                    X_co.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                y_pred_co = y_pred_co.reshape((-1, 3))
            ...
            if os.path.isfile(desc_dir+'Descs_oh_'+str(fr)+'.csv'):
                flag_oh = True
                descs_X_oh = np.loadtxt(
                    desc_dir+'Descs_oh_'+str(fr)+'.csv', delimiter=',')
                X_oh = torch.from_numpy(descs_X_oh.astype(np.float32)).clone()
                y_pred_oh = model_oh_2(
                    X_oh.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                y_pred_oh = y_pred_oh.reshape((-1, 3))
            ...
            if os.path.isfile(desc_dir+'Descs_o_'+str(fr)+'.csv'):
                flag_o = True
                descs_X_o = np.loadtxt(
                    desc_dir+'Descs_o_'+str(fr)+'.csv', delimiter=',')
                X_o = torch.from_numpy(descs_X_o.astype(np.float32)).clone()
                y_pred_o = model_o_2(
                    X_o.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                y_pred_o = y_pred_o.reshape((-1, 3))
            ...
            if os.path.isfile(desc_dir+'Descs_cc_'+str(fr)+'.csv'):
                flag_cc = True
                descs_X_cc = np.loadtxt(
                    desc_dir+'Descs_cc_'+str(fr)+'.csv', delimiter=',')
            ...
            if os.path.isfile(desc_dir+'Descs_coc_'+str(fr)+'.npy') and model_coc_2 != None:
                ...
                X_coc = torch.from_numpy(descs_X_coc.astype(
                    np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
                y_pred_coc = model_coc_2(
                    X_coc.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                y_pred_coc = y_pred_coc.reshape((-1, 3))
                del descs_X_coc
                # sum_dipole += np.sum(y_pred_coc,axis=0) # total dipoleはとりあえず無視
                if var_pre.save_truey:
                    np.save(var_pre.desc_dir+"y_pred_coc_" +
                            str(fr)+".npy", y_pred_coc)
            if os.path.isfile(desc_dir+'Descs_coh_'+str(fr)+'.npy') and model_coh_2 != None:
                descs_X_coh = np.load(desc_dir+'Descs_coh_'+str(fr)+'.npy')
                X_coh = torch.from_numpy(descs_X_coh.astype(
                    np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
                y_pred_coh = model_coh_2(
                    X_coh.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                y_pred_coh = y_pred_coh.reshape((-1, 3))
                del descs_X_coh
                # sum_dipole += np.sum(y_pred_coh,axis=0) # total dipoleはとりあえず無視
                if var_pre.save_truey:
                    np.save(var_pre.desc_dir+"y_pred_coh_" +
                            str(fr)+".npy", y_pred_coh)
            # !! <<< ここまでCOH/COC <<<
            # print("DEBUG :: shape ch/co/oh/o :: {0} {1} {2} {3}".format(np.shape(y_pred_ch),np.shape(y_pred_co),np.shape(y_pred_oh),np.shape(y_pred_o)))
            # if fr == 0:
            #     print("y_pred_ch ::", y_pred_ch)
            ...
            if flag_ch:
                sum_dipole += np.sum(y_pred_ch, axis=0)
            if flag_cc:
        ...
        def predict_dipole(fr, desc_dir):
            ...
            if model_cc_2 != None:
                y_pred_cc = y_pred_cc.reshape((-1, 3))
            y_pred_o = y_pred_o.reshape((-1, 3))
            print("DEBUG :: shape ch/co/oh/o :: {0} {1} {2} {3}".format(np.shape(
                y_pred_ch), np.shape(y_pred_co), np.shape(y_pred_oh), np.shape(y_pred_o)))
            if fr == 0:
                print("y_pred_ch ::", y_pred_ch)
                print("y_pred_co ::", y_pred_co)
                print("y_pred_oh ::", y_pred_oh)
        ...
        # TODO :: hard code :: 今は4つの結合種があるのでこうしているが，本来はこれではダメ
        num_structure = int(count_csv/var_pre.bondspecies)
        print(" ------------------ ")
        ...
        result_dipole = joblib.Parallel(n_jobs=-1, verbose=50)(joblib.delayed(
            predict_dipole_mode1)(fr, var_pre.desc_dir) for fr in range(num_structure))
        import numpy as np
    ...
    if if_calc_descripter and if_calc_predict:
        ...
        if var_des.descmode == "1":
            ...
            def calc_descripter_frame_and_predict_dipole(atoms_fr, fr, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS):
                ...
                if model_ch_2 != None:
                    Descs_ch = DESC.calc_bond_descripter_at_frame(
                        atoms_fr, list_bond_centers, itp_data.bond_index['CH_1_bond'], var_des.desctype)
                    X_ch = torch.from_numpy(Descs_ch.astype(
                        np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
                    y_pred_ch = model_ch_2(
                        X_ch.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                    y_pred_ch = y_pred_ch.reshape((-1, 3))
                ...
                if model_co_2 != None:
                    Descs_co = DESC.calc_bond_descripter_at_frame(
                        atoms_fr, list_bond_centers, itp_data.bond_index['CO_1_bond'], var_des.desctype)
                    X_co = torch.from_numpy(Descs_co.astype(
                        np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
                    y_pred_co = model_co_2(
                        X_co.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                    y_pred_co = y_pred_co.reshape((-1, 3))
                ...
                if model_oh_2 != None:
                    Descs_oh = DESC.calc_bond_descripter_at_frame(
                        atoms_fr, list_bond_centers, itp_data.bond_index['OH_1_bond'], var_des.desctype)
                    X_oh = torch.from_numpy(Descs_oh.astype(
                        np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
                    y_pred_oh = model_oh_2(
                        X_oh.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                    y_pred_oh = y_pred_oh.reshape((-1, 3))
                ...
                if model_cc_2 != None:
                    Descs_cc = DESC.calc_bond_descripter_at_frame(
                        atoms_fr, list_bond_centers, itp_data.bond_index['CC_1_bond'], var_des.desctype)
                    X_cc = torch.from_numpy(Descs_cc.astype(
                        np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
                    y_pred_cc = model_cc_2(
                        X_cc.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                    y_pred_cc = y_pred_cc.reshape((-1, 3))
                ...
                if model_o_2 != None:
                    Descs_o = DESC.calc_lonepair_descripter_at_frame(
                        atoms_fr, list_mol_coords, itp_data.o_list, 8, var_des.desctype)
                    X_o = torch.from_numpy(Descs_o.astype(
                        np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
                    y_pred_o = model_o_2(
                        X_o.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                    y_pred_o = y_pred_o.reshape((-1, 3))
            ...
            if not __debug__:
                ...
                for i in range(ave):
                    print(
                        "now we are in loop {}/i  :: {}/ave {}/res".format(i, ave, res))
                    print(
                        "len(traj[i*cpu_size:(i+1)*cpu_size]) :: {}".format(len(traj[i*cpu_size:(i+1)*cpu_size])))
                    # trajをcpu_sizeだけ読んでjoblibに渡す
                    for fr, atoms_fr in enumerate(traj[i*cpu_size:(i+1)*cpu_size]):
                        result_dipole = calc_descripter_frame_and_predict_dipole(
                            atoms_fr, fr, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS)
                        print(result_dipole)
            ...
            if sys.getsizeof(traj)/1000 > 0:  # < 100: # 100KB以下の場合は通常のjoblibを使う．
                print(
                    " xyz file is not very large, and we use normal joblib calculation !! ")
                ...
                result = joblib.Parallel(n_jobs=OMP_NUM_THREADS, verbose=50)(joblib.delayed(calc_descripter_frame_and_predict_dipole)(
                    atoms_fr, fr, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS) for fr, atoms_fr in enumerate(traj))
                # xyzデータと双極子データを取得
                result_ase = [i[0] for i in result]
                ...
                print(
                    " xyz file is very large, and we induce different calculation type !! ")
                # num_trajをco_workersで分割して処理するので，繰り返し回数とあまりを計算する（mpiと同じ処理）
                ...
                for i in range(ave):
                    ...
                    result_dipole = joblib.Parallel(n_jobs=-1, verbose=50)(joblib.delayed(calc_descripter_frame_and_predict_dipole)(
                        atoms_fr, fr, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS) for fr, atoms_fr in enumerate(tmp_traj))
                    print(" result_dipole is ... {}".format(result_dipole))
                    result_dipole = np.array(result_dipole)
                    print(" result_dipole is ... {}".format(result_dipole))
                    # np.save(var_des.savedir+"/wannier_dipole.npy", result_dipole)
                    np.save(var_des.savedir+"/result_dipole_" +
                            str(i)+".npy", result_dipole)
                    print(" finish save step :: {}".format(i))
                # 最後のあまりの部分
                print(" Now starting final res part !!")
                result_dipole = joblib.Parallel(n_jobs=-1, verbose=50)(joblib.delayed(calc_descripter_frame_and_predict_dipole)(
                    atoms_fr, fr, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS) for fr, atoms_fr in enumerate(traj[ave*cpu_size:]))
                print(" np.shape(result_dipole) == ave ?? :: {}".format(
                    np.shape(result_dipole)))
                result_dipole = np.array(result_dipole)
                # np.save(var_des.savedir+"/wannier_dipole.npy", result_dipole)
                np.save(var_des.savedir+"/result_dipole_res.npy", result_dipole)
        ...
        if var_des.descmode == "2":
            ...
            def calc_descripter_frame(atoms_fr, wannier_fr, fr, itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS):
                ...
                # TODO :: そうしておけば後ろでatomsを使う時にmicのことを気にしなくて良い（？）ので楽かも．
                results = ASIGN.aseatom_to_mol_coord_bc(
                    atoms_fr, itp_data, bonds_list)
                list_mol_coords, list_bond_centers = results

                # wcsをbondに割り当て，bondの双極子まで計算
                results_mu = ASIGN.calc_mu_bond_lonepair(
                    wannier_fr, atoms_fr, bonds_list, itp_data, double_bonds)
                list_mu_bonds, list_mu_pai, list_mu_lpO, list_mu_lpN, list_bond_wfcs, list_pi_wfcs, list_lpO_wfcs, list_lpN_wfcs = results_mu
                # wannnierをアサインしたase.atomsを作成する
                mol_with_WC = cpmd.asign_wcs.make_ase_with_WCs(atoms_fr.get_atomic_numbers(
                ), NUM_MOL, UNITCELL_VECTORS, list_mol_coords, list_bond_centers, list_bond_wfcs, list_pi_wfcs, list_lpO_wfcs, list_lpN_wfcs)
                ...
                if len(itp_data.bond_index['CH_1_bond']) != 0 and model_ch_2 != None:
                    Descs_ch = DESC.calc_bond_descripter_at_frame(
                        atoms_fr, list_bond_centers, itp_data.bond_index['CH_1_bond'], var_des.desctype)
                    X_ch = torch.from_numpy(
                        Descs_ch.astype(np.float32)).clone()
                    y_pred_ch = model_ch_2(
                        X_ch.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()   # 予測
                    # !! ここは形としては(NUM_MOL*len(bond_index),3)となるが，予測だけする場合NUM_MOLの情報をgetできないのでreshape(-1,3)としてしまう．
                    y_pred_ch = y_pred_ch.reshape((-1, 3))
                    ...
                    if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                        # 予測値の保存
                        np.save(var_pre.desc_dir+"/y_pred_ch_" +
                                str(fr)+".npy", y_pred_ch)
                if len(itp_data.bond_index['CO_1_bond']) != 0 and model_co_2 != None:
                    ...
                    if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                        # 予測値の保存
                        np.save(var_pre.desc_dir+"/y_pred_co_" +
                                str(fr)+".npy", y_pred_co)
                if len(itp_data.bond_index['OH_1_bond']) != 0 and model_oh_2 != None:
                    ...
                    if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                        # 予測値の保存
                        np.save(var_pre.desc_dir+"/y_pred_oh_" +
                                str(fr)+".npy", y_pred_oh)
                if len(itp_data.bond_index['CC_1_bond']) != 0 and model_cc_2 != None:
                    ...
                    if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                        # 予測値の保存
                        np.save(var_pre.desc_dir+"/y_pred_cc_" +
                                str(fr)+".npy", y_pred_cc)
                if len(itp_data.o_list) != 0 and model_o_2 != None:
                    ...
                    if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                        # 予測値の保存
                        np.save(var_pre.desc_dir+"/y_pred_o_" +
                                str(fr)+".npy", y_pred_o)
                # !! >>>> ここからCOH/COC >>>
                if len(itp_data.o_list) != 0 and model_coc_2 != None:
                    # TODO :: このままだと通常のo_listを使ってしまっていてまずい．
                    # TODO :: ちゃんとcohに対応したo_listを作るようにする．
                    Descs_coc = DESC.calc_lonepair_descripter_at_frame(
                        atoms_fr, list_mol_coords, itp_data.o_list, 8, var_des.desctype)
                    X_coc = torch.from_numpy(Descs_coc.astype(
                        np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
                    y_pred_coc = model_coc_2(
                        X_coc.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                    y_pred_coc = y_pred_coc.reshape((-1, 3))
                ...
                if len(itp_data.o_list) != 0 and model_coh_2 != None:
                    ...
                    Descs_coh = DESC.calc_lonepair_descripter_at_frame(
                        atoms_fr, list_mol_coords, itp_data.o_list, 8, var_des.desctype)
                    X_coh = torch.from_numpy(Descs_coh.astype(
                        np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
                    y_pred_coh = model_coh_2(
                        X_coh.reshape(-1, nfeatures).to(device)).to("cpu").detach().numpy()
                    y_pred_coh = y_pred_coh.reshape((-1, 3))
                    del Descs_coh
                    # total dipoleはとりあえず無視
                ...
                if var_pre.save_truey:  # 予測値をボンドごとに保存する場合
                    ...
                    np.save(var_pre.desc_dir+"/y_true_ch_" +
                            str(fr)+".npy", True_y_ch)
                    np.save(var_pre.desc_dir+"/y_true_co_" +
                            str(fr)+".npy", True_y_co)
                    np.save(var_pre.desc_dir+"/y_true_oh_" +
                            str(fr)+".npy", True_y_oh)
                    np.save(var_pre.desc_dir+"/y_true_cc_" +
                            str(fr)+".npy", True_y_cc)
                    np.save(var_pre.desc_dir+"/y_true_o_" +
                            str(fr)+".npy", True_y_o)
            ...
            if var_des.step != None:  # stepが決まっている場合はこちらで設定してしまう．
                print("STEP is manually set :: {}".format(var_des.step))
                traj = traj[:var_des.step]
            result_dipoles = joblib.Parallel(n_jobs=-1, verbose=50)(joblib.delayed(calc_descripter_frame)(atoms_fr, wannier_fr, fr,
                                                                                                          itp_data, NUM_MOL, NUM_MOL_ATOMS, UNITCELL_VECTORS) for fr, (atoms_fr, wannier_fr) in enumerate(zip(traj, wannier_list)))
            # !! debug
            print("len(result_dipoles) :: {}".format(len(result_dipoles)))
            print("len(result_dipoles[0]) :: {}".format(
                len(result_dipoles[0])))
            print("len(result_dipoles[1]) :: {}".format(
                len(result_dipoles[1])))
            ...
            make_merge_predy(
                len(traj), NUM_MOL, itp_data.bond_index['OH_1_bond'], var_des.savedir, "oh")
            make_merge_predy(len(traj), NUM_MOL,
                             itp_data.o_list,        var_des.savedir, "o")
            make_merge_predy(len(traj), NUM_MOL,
                             itp_data.coc_index,     var_des.savedir, "coc")
            make_merge_predy(len(traj), NUM_MOL,
