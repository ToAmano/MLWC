...
def calc_velocity(traj: list[ase.Atoms], timestep_fs: float = 1) -> np.ndarray:
    ...
    diff_pbc = pbc(pbc_3d).compute_pbc(
        vectors_array=diff_coord, cell=traj[0].get_cell()
    )  # [bondcent,Atom,3]
...
def calc_atom_velocity(
    traj: list[ase.Atoms], atomic_number: int, timestep_fs: float = 1
) -> np.ndarray:
    """calculate velocity of each MD frame
...
def calc_momentum(
    traj: list[ase.Atoms], NUM_ATOM: int, timestep_fs: float = 1
) -> np.ndarray:
    """calculate velocity of each MD frame
    ...
    atomic_index = np.where(
        (traj_atomic_number == 1)
        | (traj_atomic_number == 6)
        | (traj_atomic_number == 8)
    )[0]
    ...
    atom_velocity = np.einsum("j,ijk->ijk", atoms_mass[:NUM_ATOM], diff_pbc) / (
        timestep_fs / 1000
    )
    return atom_velocity


def calc_com_velocity(
    traj: list[ase.Atoms], NUM_ATOM_PER_MOL: int, timestep_fs: float = 1
) -> np.ndarray:
    ...
    MOL_WITHOUT_WC_LIST: list[bool] = np.where(
        (atoms_1mol == 1) | (atoms_1mol == 6) | (atoms_1mol == 8)
    )[0]
    ...
    com_coordinate = (
        np.einsum("k,ijkl->ijl", atoms_mass, atom_coordinate)
        / NUM_ATOM_PER_MOL_WITHOUT_WC
    )
    ...
    for atom_id in range(NUM_ATOM):
        acf_x = (
            sm.tsa.stattools.acf(
                atom_velocity[:, atom_id, 0], fft=True, nlags=len(atom_velocity)
            )
            * np.std(atom_velocity[:, atom_id, 0])
            * np.std(atom_velocity[:, atom_id, 0])
        )
        acf_y = (
            sm.tsa.stattools.acf(
                atom_velocity[:, atom_id, 1], fft=True, nlags=len(atom_velocity)
            )
            * np.std(atom_velocity[:, atom_id, 1])
            * np.std(atom_velocity[:, atom_id, 1])
        )
        acf_z = (
            sm.tsa.stattools.acf(
                atom_velocity[:, atom_id, 2], fft=True, nlags=len(atom_velocity)
            )
            * np.std(atom_velocity[:, atom_id, 2])
            * np.std(atom_velocity[:, atom_id, 2])
        )
        # !! 内積なので足し上げるのが正解
...
def average_vdos_specify_index(
    acf: np.ndarray, index: list[int], num_atoms_per_mol: int
):
    """
    indexを自分で指定する場合
    acf[atom_id] = acf
