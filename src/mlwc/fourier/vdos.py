...
def calc_momentum(traj: list[ase.Atoms], NUM_ATOM: int, timestep_fs: float = 1) -> np.ndarray:
    ...
    atomic_index = np.where((traj_atomic_number == 1) | (
        traj_atomic_number == 6) | (traj_atomic_number == 8))[0]

    # atomic masses for single molecule
    atoms_mass = traj[0][atomic_index].get_masses()
    ...
    atom_velocity = np.einsum(
        "j,ijk->ijk", atoms_mass[:NUM_ATOM], diff_pbc)/(timestep_fs/1000)
    return atom_velocity
...
def calc_com_velocity(traj: list[ase.Atoms], NUM_ATOM_PER_MOL: int, timestep_fs: float = 1) -> np.ndarray:
    ...
    MOL_WITHOUT_WC_LIST: list[bool] = np.where(
        (atoms_1mol == 1) | (atoms_1mol == 6) | (atoms_1mol == 8))[0]
    NUM_ATOM_PER_MOL_WITHOUT_WC: int = len(MOL_WITHOUT_WC_LIST)
    # atomic masses for single molecule
    atoms_mass = traj[0][MOL_WITHOUT_WC_LIST].get_masses()
    logger.info(
        f"NUM_ATOM_PER_MOL_WITHOUT_WC :: {NUM_ATOM_PER_MOL_WITHOUT_WC}")

    # 速度の初期化
    atom_coordinate = np.zeros(
        [len(traj), NUM_MOL, NUM_ATOM_PER_MOL_WITHOUT_WC, 3])
    for counter, atoms in enumerate(traj):  # loop over frame
        # reshape coordinate to [NUM_MOL,NUM_ATOM_PER_MOL_WITHOUT_WC,3]
        atom_coordinate[counter] = atoms.get_positions()[atomic_index].reshape(
            NUM_MOL, NUM_ATOM_PER_MOL_WITHOUT_WC, 3)
...
def calc_vel_acf(atom_velocity: np.ndarray):
    ...
    for atom_id in range(NUM_ATOM):
        acf_x = sm.tsa.stattools.acf(atom_velocity[:, atom_id, 0], fft=True, nlags=len(
            atom_velocity))*np.std(atom_velocity[:, atom_id, 0]) * np.std(atom_velocity[:, atom_id, 0])
        acf_y = sm.tsa.stattools.acf(atom_velocity[:, atom_id, 1], fft=True, nlags=len(
            atom_velocity))*np.std(atom_velocity[:, atom_id, 1]) * np.std(atom_velocity[:, atom_id, 1])
        acf_z = sm.tsa.stattools.acf(atom_velocity[:, atom_id, 2], fft=True, nlags=len(
            atom_velocity))*np.std(atom_velocity[:, atom_id, 2]) * np.std(atom_velocity[:, atom_id, 2])
        # !! 内積なので足し上げるのが正解
...
def average_vdos_atomic_species(acf: np.ndarray, atoms: ase.Atoms, atomic_number: int):
    ...
    '''
    # indexを取得([0]が必要)
    atomic_index: list[int] = np.where(
        atoms.get_atomic_numbers() == atomic_number)[0]
...
def average_vdos_specify_index(acf: np.ndarray, index: list[int], num_atoms_per_mol: int):
    ...
    atomic_index = [
        j+i*num_atoms_per_mol for j in index for i in range(NUM_MOL)]
...
def calc_vdos(acf: np.ndarray, timestep: float) -> pd.DataFrame:
    ...
    # subtract vdos(omega=0) to assure vdos(0)=0
