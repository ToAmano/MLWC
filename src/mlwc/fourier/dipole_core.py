...
def add_charges(atoms_list, charge_list):
    ...
    if not len(atoms_list[0].get_chemical_symbols()) == len(charge_list[0]):
        ...
        print(
            "# of atoms for atoms_list :: ", len(atoms_list[0].get_chemical_symbols())
        )
...
def calc_dipoles(atoms_list):
    ...
    for i in range(len(atoms_list)):
        tmp_dipole = np.einsum(
            "i,ij -> j",
            atoms_list[i].get_initial_charges(),
...
def calc_eps0(dipole, unitcell_vector, start=0, stop=-1, T=300):
    ...
    kbT = kb * T
    V = (
        np.dot(unitcell_vector[0], np.cross(unitcell_vector[1], unitcell_vector[2]))
        * A3
    )  # 11.1923*11.1923*11.1923 * A3
    print(" -------------- ")
    print(" volume :: ", V)
    print("")
    # N = int(len(ms))
    ...
    # 実際のeps0の計算
    eps_0 = 1.0 + ((np.mean(dMx**2 + dMy**2 + dMz**2)) * debye**2) / (
        3.0 * V * kbT * eps0
    )
...
class diel_function:
    ...
    def __init__(self, freq_thz, ffteps1, ffteps2, step: int = 1):
        """_summary_
        ...
        self.diel_df["freq_thz"] = freq_thz
        self.diel_df["freq_kayser"] = freq_thz * 33.3
        self.diel_df["real_diel"] = ffteps1
        self.diel_df["imag_diel"] = ffteps2
        self.diel_df["alphan"] = raw_calculate_absorption(
            self.diel_df
    ...
    def calc_alpha(self):
        ...
        window = np.ones(self.step) / self.step
        alpha = np.convolve(
            refractive_index["imag_ref_index"]
            * refractive_index["freq_kayser"]
            / 33.3
            * 400
            * 3.14
            / 3,
...
def raw_calculate_refractiveindex_pandas(eps_df, step: int = 1):
    ...
    if step < 1:
        print("ERROR :: step must be larger than 1")
        return 0
    return raw_calculate_refractiveindex(
        eps_df["freq_kayser"], eps_df["real_diel"], eps_df["imag_diel"], step
    )
...
def raw_calculate_alpha(refractive_df, step: int = 1):
    ...
    return np.convolve(
        refractive_df["imag_ref_index"]
        * refractive_df["freq_kayser"]
        / 33.3
        * 400
        * 3.14
        / 3,
