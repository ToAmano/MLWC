...
from mlwc.fourier.windowfunction import apply_windowfunc_oneside
...
def raw_calc_fourier_window(
    ...
    Returns:
        _type_: _description_
    """
    fft_data_with_windowfunction = apply_windowfunc_oneside(fft_data, window)
...
def raw_calc_fourier_window_no_normalize(
    ...
    Returns:
        _type_: _description_
    """
    fft_data_with_windowfunction = apply_windowfunc_oneside(fft_data)
...
def calc_mean_M(dipole_array: np.array):
    # 予測値
...
def calc_mean_M2(dipole_array: np.array):
    # 予測値
    dMx_pred = dipole_array[:, 0]  # -cell_dipoles_pred[0,0]
    dMy_pred = dipole_array[:, 1]  # -cell_dipoles_pred[0,1]
    dMz_pred = dipole_array[:, 2]  # -cell_dipoles_pred[0,2]
...
def raw_calc_eps0_dielconst(
    ...
    mean_M2: float = calc_mean_M2(cell_dipoles_pred)
    # np.mean(dMx_pred)**2+np.mean(dMy_pred)**2+np.mean(dMz_pred)**2    # <M>^2
...
def raw_calc_derivative_spectrum(
    ...
    fft_data = apply_windowfunc_oneside(acf_x + acf_y + acf_z, window="hann")
    # acfのfourier変換を計算
