...
class ReadCP(dataio.read_core.custom_traj):
    ...
    def __init__(self, filename: str):
        super().__init__(
            atoms_list=raw_transform_xyz(
                xyz_filename=filename,
                unitcell_vector=raw_read_unitcell_vector(filename),
                JUMP=False,
            ),
            unitcell_vector=raw_read_unitcell_vector(filename),
            filename=filename,
...
class ReadXDATCAR(dataio.read_core.custom_traj):
    ...
    def __init__(self, filename: str):
        super().__init__(
            atoms_list=raw_transform_xdatcar(xdatcar_filename=filename),
            unitcell_vector=raw_transform_xdatcar(xdatcar_filename=filename)[
                0
            ].get_cell(),
            filename=filename,
        )
...
class ReadPOS(dataio.read_core.custom_traj):
    ...
    def __init__(self, filename: str, pwin: str):
        ...
        # make atoms
        atoms_list = dataio.read_core.raw_make_atomslist(pos_list, tmp_cell, tmp_symbol)
    ...
    if JUMP == False:
        ...
        for i in np.arange(1, len(atoms_list)):
            ...
            if np.any(np.abs(coord_before - coord) > cell_check / 2):
                ...
                # jumpする場合には格子定数を追加する．
                tmp1 = np.where(
                    coord - coord_before > cell_check / 2, coord, coord + cell_check
                )
                tmp2 = np.where(
                    coord - coord_before < -cell_check / 2, tmp1, tmp1 - cell_check
                )
