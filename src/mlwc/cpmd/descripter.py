...
def cutoff_func(Rij: np.array, Rcs: float, Rc: float) -> np.array:
    ...
    s = np.where(
        ...
        np.where(
            Rij < Rc,
            (1 / Rij) * (0.5 * np.cos(np.pi * (Rij - Rcs) / (Rc - Rcs)) + 0.5),
            0,
        ),
    )
    return s
...
def cutoff_func_torch(Rij: torch.Tensor, Rcs: float, Rc: float) -> torch.Tensor:
    ...
    s = torch.where(
        ...
        torch.where(
            Rij < Rc,
            (1 / Rij) * (0.5 * torch.cos(torch.pi * (Rij - Rcs) / (Rc - Rcs)) + 0.5),
            0,
        ),
    )
    return s
...
class descripter:
    ...
    def calc_lonepair_descripter_at_frame_type2(
        ...
        if len(at_list) != 0:  # 中身が0でなければ計算を実行
            if desctype == "old":
                raise ValueError(
                    "calc_lonepair_descripter_at_frame_type2 :: desctype = old is not supported for torch descriptors!!"
                )
...
def _raw_make_atoms(bond_center, atoms: ase.Atoms, UNITCELL_VECTORS):
    ...
    WBC = ase.Atoms(atom_id, positions=pos, cell=UNITCELL_VECTORS, pbc=[1, 1, 1])
    return WBC
...
def _calc_descripter_for_index(
    ...
    if (
        ...
        tmp = (
            sorted_cutoff[:, np.newaxis] * sorted_drs / sorted_d[:, np.newaxis]
        )  # 3成分cutoff*(x/r,y/r,z/r)を計算
...
def raw_get_desc_bondcent(
    ...
    dist_wVec = raw_get_distances_mic(
        atoms_w_bc, 0, at_list, mic=True, vector=True
    )  # 0-0間距離も含まれる
...
def raw_get_desc_bondcent_allinone(
    atoms: ase.Atoms,
    bond_center,
    UNITCELL_VECTORS,
    NUM_MOL_ATOMS: int,
    ...
    at_list = np.arange(
        len(atoms_w_bc)
    )  # !! 024/1/11 numpyに変更した．デバックが必要．
    # dist_wVec = atoms_w_bc.get_distances(0,at_list,mic=True,vector=True)  #0-0間距離も含まれる
    dist_wVec = raw_get_distances_mic(
        atoms_w_bc, 0, at_list, mic=True, vector=True
    )  # 0-0間距離も含まれるので，先頭が0になる．
    # at_nums = atoms_w_bc.get_atomic_numbers()
...
# !! added at 2024/1/11
def get_desc_bondcent_allinone_torch(
    atoms: ase.Atoms,
    bond_centers,
    UNITCELL_VECTORS,
    Rcs: float = 4.0,
    ...
    if dij_C_all.size(1) < 4 * MaxAt:
        padding = torch.zeros(dij_C_all.size(0), 4 * MaxAt - dij_C_all.size(1)).to(
            device
        )
    ...
    s = torch.where(
        ...
        torch.where(
            d < Rc,
            (1 / d) * (0.5 * torch.cos(torch.pi * (d - Rcs) / (Rc - Rcs)) + 0.5),
            0,
        ),
    )
    order_indx1 = torch.argsort(s, descending=True)  # sの大きい順に並べる
    ...
    if dij_H_all.size(1) < 4 * MaxAt:
        padding = torch.zeros(dij_H_all.size(0), 4 * MaxAt - dij_H_all.size(1)).to(
            device
        )
    ...
    if dij_O_all.size(1) < 4 * MaxAt:
        padding = torch.zeros(dij_O_all.size(0), 4 * MaxAt - dij_O_all.size(1)).to(
            device
        )
...
def raw_get_desc_lonepair(
    atoms, lonepair_coord, mol_id, UNITCELL_VECTORS, NUM_MOL_ATOMS: int
):
...
def raw_get_desc_lonepair_allinone(
    atoms,
    lonepair_coord,
    UNITCELL_VECTORS,
    NUM_MOL_ATOMS: int,
...
def raw_get_desc_lonepair_allinone_torch(
    atoms: ase.Atoms,
    lonepair_coords: np.array,
    UNITCELL_VECTORS: np.array,
    Rcs: float = 4.0,
    Rc: float = 6.0,
    MaxAt: int = 24,
    ...
    list_mol_coords = torch.tensor(list_mol_coords).to(device)
    list_atomic_nums = torch.tensor(list_atomic_nums).to(device)
    lonepair_coords = lonepair_coords.reshape(
        -1, 3
    )  # reshape [num_mol,o_index,coord] to 2D array ( これが必要？)
    lonepair_coords = torch.tensor(np.array(lonepair_coords, dtype="float32")).to(
        device
    )
    ...
    matB = lonepair_coords[None, :, :].repeat(
    ...
    dd = tmp.shape
    drs = tmp[torch.sum(tmp**2, axis=2) > 0.0001].reshape(
        (dd[0], -1, 3)
...
def find_specific_ringmu(list_mu_bonds, list_mu_pai, ring_index):
    ring_mu_mol = []
    # ボンドセンターの座標と双極子をappendする．
    for mol_mu_bond, mol_mu_pai in zip(
        list_mu_bonds, list_mu_pai
    ):  # UnitCellの分子ごとに分割
...
def find_specific_lonepair(
    list_mol_coords, aseatoms, atomic_index: int, NUM_MOL: int
) -> np.array:
    """
    ...
    for atOs, mol_coords in zip(at_list, list_mol_coords):
        ...
        cent_mol.append(
            mol_coords[atOs]
        )  # ここはatomic_indexに対応した原子（酸素なら8）の座標をappendする
    # reshape
...
def raw_calc_bond_descripter_at_frame(
    atoms_fr,
    ...
    Descs = []
    cent_mol = find_specific_bondcenter(
    ...
    if len(bond_index) != 0:  # 中身が0でなければ計算を実行
        if desctype == "old":
            ...
            for bond_center in cent_mol:
                mol_id = (
                    i % NUM_MOL // len(bond_index)
                )  # 対応する分子ID（mol_id）を出すように書き直す．ボンドが1分子内に複数ある場合，その数で割らないといけない．（メタノールならCH結合が3つあるので3でわる）
                Descs.append(
                    raw_get_desc_bondcent(
                        atoms_fr, bond_center, mol_id, UNITCELL_VECTORS, NUM_MOL_ATOMS
                    )
                )
                i += 1
        elif desctype == "allinone":
            ...
            Descs = get_desc_bondcent_allinone_torch(
                atoms_fr, cent_mol, UNITCELL_VECTORS, Rcs, Rc, MaxAt
            )
    return np.array(Descs)
...
# !! COHボンド対応のTrue_y計算用
def raw_calc_coh_bondmu_descripter_at_frame(
    list_mu_bonds: np.array,
    list_mu_lp: np.array,
    coh_index: list,
    co_bond_index: list[int],
    oh_bond_index: list[int],
) -> np.array:
...
def raw_calc_coc_bondmu_descripter_at_frame(
    list_mu_bonds: np.array,
    list_mu_lp: np.array,
    coc_index: list,
    co_bond_index: list[int],
...
def raw_find_atomic_index(aseatoms, atomic_index: int, NUM_MOL: int):
    ...
    list_atomic_nums = list(
        np.array(aseatoms.get_atomic_numbers()).reshape(NUM_MOL, -1)
    )  # atomic_numbersを分子ごとにreshape
    at_list = [
        np.argwhere(js == atomic_index).reshape(-1).tolist() for js in list_atomic_nums
    ]  # atomic_indexに対応するindexを返す
    return at_list


def raw_calc_lonepair_descripter_at_frame(
    atoms_fr,
    list_mol_coords,
    at_list,
    NUM_MOL: int,
    ...
    list_lonepair_coords = find_specific_lonepair(
        list_mol_coords, atoms_fr, atomic_index, NUM_MOL
    )  # atomic_indexに対応した原子の座標を抜き取る
    ...
    if len(at_list) != 0:  # 中身が0でなければ計算を実行
        if desctype == "old":
            ...
            for bond_center in list_lonepair_coords:
                mol_id = (
                    i % NUM_MOL // len(at_list)
                )  # 対応する分子ID（mol_id）を出すように書き直す．（1分子内のO原子の数（len(at_list）でわって分子idを出す）
                Descs.append(
                    raw_get_desc_lonepair(
                        atoms_fr, bond_center, mol_id, UNITCELL_VECTORS, NUM_MOL_ATOMS
                    )
                )
                i += 1
            ...
            # using Torch
            Descs = raw_get_desc_lonepair_allinone_torch(
                atoms_fr, list_lonepair_coords, UNITCELL_VECTORS, Rcs, Rc, MaxAt
            )
    return np.array(Descs)


def raw_calc_lonepair_descripter_at_frame2(
    atoms_fr,
    list_mol_coords,
    at_list,
    NUM_MOL: int,
    ...
    # 実際の記述子の計算
    Descs = [
        raw_get_desc_lonepair_allinone(
            atoms_fr, bond_center, UNITCELL_VECTORS, NUM_MOL_ATOMS
        )
        for bond_center in list_lonepair_coords
    ]
