...
def make_ase_with_BCs(
    ase_atomicnumber, NUM_MOL, UNITCELL_VECTORS, list_mol_coords, list_bond_centers
):
    """
    ...
    for mol_r, mol_at, mol_bc in zip(
        list_mol_coords, list_atomic_nums, list_bond_centers
    ):
    ...
    aseatoms_with_BC = Atoms(
        new_atomic_num, positions=new_coord, cell=UNITCELL_VECTORS, pbc=[1, 1, 1]
    )
    return aseatoms_with_BC


def make_ase_with_WCs(
    ase_atomicnumber,
    NUM_MOL,
    UNITCELL_VECTORS,
    list_mol_coords,
    ...
    list_atomic_nums = list(np.array(ase_atomicnumber).reshape(NUM_MOL, -1))
    for mol_r, mol_at, mol_wc, mol_bc, mol_Dwc, mol_lpO, mol_lpN in zip(
        list_mol_coords,
        list_atomic_nums,
        list_bond_wfcs,
        list_bond_centers,
...
class asign_wcs:
    ...
    # atom_coordとwcsからase.atomsを作る
    def make_aseatoms_from_wc(self, atom_coord: np.array, wfc_list):
    ...
    def find_all_bonds(self, wfc_list, list_bond_centers, picked_wfcs):
        return raw_find_all_bonds(
            wfc_list, list_bond_centers, picked_wfcs, self.UNITCELL_VECTORS
        )
...
def raw_aseatom_to_mol_coord_bc(
    ...
    mol_ats = [
        [int(at + NUM_MOL_ATOMS * indx) for at in mol_at0] for indx in range(NUM_MOL)
    ]
    # * ? bond indexもJ番目のボンドに対応させる
    unit_cell_bonds = []
    for indx in range(NUM_MOL):
        unit_cell_bonds.append(
            [
                [
                    int(b_pair[0] + NUM_MOL_ATOMS * indx),
                    int(b_pair[1] + NUM_MOL_ATOMS * indx),
                ]
                for b_pair in bonds_list
            ]
        )
    ...
    for j in range(NUM_MOL):  # 全ての分子に対する繰り返し．
        ...
        mol_coords, bond_centers = raw_calc_mol_coord_and_bc_mic_onemolecule(
            mol_inds, bonds_list_j, ase_atoms, itp_data
...
def get_distances_numpy(
    pos_origin: np.ndarray, pos_destination: np.ndarray, cell
) -> np.ndarray:
...
def get_desc_bondcent_yamazaki(atoms, Rcs, Rc, MaxAt, bond_center, mol_id):
    ...
    s = np.where(
        ...
        1 / d,
        np.where(
            d < Rc, (1 / d) * (0.5 * np.cos(np.pi * (d - Rcs) / (Rc - Rcs)) + 0.5), 0
        ),
    )
...
def raw_calc_mol_coord_and_bc_mic_onemolecule(
    mol_inds, bonds_list_j, aseatoms: ase.Atoms, itp_data
):
    """
    ...
    # 全てのボンドセンターの座標を取得する．
    bond_centers = [
        R0 + (distance_vectors[l[0]] + distance_vectors[l[1]]) / 2.0
...
def raw_find_pi(
    ...
    # 先頭原子とWCsの距離ベクトル
    # wfc_vectors = atom_wan.get_distances(0,range(1,num_element),mic=True,vector=True) #ワニエ中心は一度CP.xを通しているから点ごとにPBCが適用されている。mic=Trueでよい。
    wfc_vectors = raw_get_distances_mic(
        atom_wan, 0, range(1, num_element), mic=True, vector=True
    )  # 上のコードと同じことをしている．
    wfc_distances = np.linalg.norm(wfc_vectors, axis=1)
    ...
    for bcs in list_bond_centers:  # 分子数に関するループ
        ...
        for bond_center_coord in bcs:  # ある分子内のボンドセンターに関するループ
            ...
            wcs_indices, mu_bond, wcs_bond = raw_find_lonepairs(
                bond_center_coord, wfc_list, 1, UNITCELL_VECTORS, picked_wfcs
            )
            picked_wfcs = picked_wfcs + list(wcs_indices)
...
def raw_calc_mu_bond_lonepair(
    ...
    list_mu_bonds, list_bond_wfcs, picked_wcs_bond = raw_find_all_bonds(
        wfc_list, list_bond_centers, picked_wfcs, UNITCELL_VECTORS
    )
