...
def get_remote_port(port=None, notebook_path=""):
    ...
    client_cm = (
        "ssh -NL localhost:{port}:localhost:{port} {username}@{hostname}".format(
...
def parse_cml_args(cmd, default_jexe):
    ...
    parser.add_argument(
        "command",
        nargs="?",
        help="command could be a trajectory filename (currently only .xyz by CP.x and XDATCAR by  VASP) or \n"
        "If not given, a notebook will be created with only nglview imported",
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=False,
        help="how to treat periodic boundary condition. If true, atoms stay in the cell, \n"
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=False,
        help=" True if want to make gif. Still experimantal. Be careful to use. \n",
    )
    ...
    parser.add_argument(
        "--clean", action="store_true", help="delete temp file after closing notebook"
    )

    parser.add_argument(
        "--auto", action="store_true", help="Run 1st cell right after openning notebook"
    )
...
def main(notebook_dict=notebook_dict, cmd=None):
    ...
    if command in ["install", "enable", "uninstall"]:
        cmds = ["jupyter", "nbextension", command, "--py", "--sys-prefix", "nglview"]
        if command == "install":
    ...
    if command is not None and command.endswith(".ipynb"):
        ...
        # これがまさに今求めているやつ
            ...
            res = subprocess.run(
                ["cp", original_dirpath + "/show_CP.py", "show_XDATCAR_tmp.py"]
            )
            ...
            res = subprocess.run(
                ["sed -i -e " + second_line + " show_XDATCAR_tmp.py"], shell=True
            )
            res = subprocess.run(
                ["sed -i -e " + first_line + " show_XDATCAR_tmp.py"], shell=True
            )
        ...
        # xyzの場合．jumpの有無も確認する．
            ...
            # dirname = os.path.dirname(os.path.abspath())
            res = subprocess.run(
                ["cp", original_dirpath + "/show_CP.py", "show_CP_tmp.py"]
            )
            ...
            # res=subprocess.run(["sed", "-i", "-e", first_line , "show_XDATCAR_tmp.py"], shell=True)
            res = subprocess.run(
                ["sed -i -e " + third_line + " show_CP_tmp.py"], shell=True
            )
            res = subprocess.run(
                ["sed -i -e " + second_line + " show_CP_tmp.py"], shell=True
            )
            res = subprocess.run(
                ["sed -i -e " + first_line + " show_CP_tmp.py"], shell=True
            )
    ...
    if not args.remote:
        ...
        cm = (
            ...
            "--notebook-dir {dirname}".format(
                jupyter=args.jexe, port=port, dirname=dirname
            )
        )
        print(
            "NOTE: make sure to open {} in your local machine\n".format(notebook_name)
        )
