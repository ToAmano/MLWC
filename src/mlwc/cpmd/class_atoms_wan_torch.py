...
def calculate_molcoord(
    atoms: ase.Atoms, bonds_list, ref_atom_index, NUM_ATOM_PER_MOL=None
):
...
def calculte_bcs(atomic_positions, bonds_list, NUM_ATOM_PER_MOL: None):
    ...
    mol_offsets = np.arange(NUM_MOL)[:, None] * NUM_ATOM_PER_MOL  # (NUM_MOL, 1)
    expanded_bonds = bonds_list[None, :, :] + mol_offsets[:, None, :]  # (NUM_MOL, M, 2)
    expanded_bonds = expanded_bonds.reshape(-1, 2)  # (NUM_MOL * M, 2)

    # 結合中点の座標を計算
    bond_centers = (
        atomic_positions[expanded_bonds[:, 0]] + atomic_positions[expanded_bonds[:, 1]]
    ) / 2
    ...
    def aseatom_to_mol_coord_bc(self):
        ...
        for j in range(self.NUM_MOL):  # 全ての分子に対する繰り返し．
            ...
            mol_coords, bond_centers = (
                cpmd.asign_wcs.raw_calc_mol_coord_and_bc_mic_onemolecule(
