...
&ATOMS
         """.format(
            max_step, timestep, self.cell_parameter
        )
    ...
    def make_bomd_restart(
        self, max_step: float = 10000, timestep: float = 40, type="default"
    ):
 ...
 &CPMD
  ...
  &END
         ...
         &ATOMS
         """.format(
            "ACCUMULATORS", max_step, timestep, self.cell_parameter
        )
 ...
 &CPMD
  ...
  &END
    ...
    def make_cpmd(
        self, max_step: float = 100000, type: str = "default", emass: float = 300
    ):
        """_summary_
        bomdではなくcpmd用の計算．
        wannierの収集を行なわない．
 ...
 &CPMD
  ...
  &END
         ...
         &ATOMS
         """.format(
            "", max_step, emass, self.cell_parameter
        )
 ...
 &CPMD
  ...
  &END
         ...
         &ATOMS
         """.format(
            "ACCUMULATORS", max_step, emass, self.cell_parameter
        )
  ...
  &END
    ...
    def write_coordinates(self, file):
        ...
        for i, index in enumerate(self.ase_atoms.get_chemical_symbols()):
            ...
            file.write(
                " {0:.10f} {1:.10f} {2:.10f} \n".format(
                    ase_atoms_position[i][0],
                    ase_atoms_position[i][1],
                    ase_atoms_position[i][2],
                )
            )
            pre_index = index
        return 0
    ...
    def write_coordinates_type2(self, file):
        ...
        sorted_atom = [
            [i[0], i[1], i[2], i[3]] for i in sorted(list_for_sort, key=lambda x: x[0])
        ]
        # print(" ソートした順番に関するインデックス")
        # print(sort_index)
        print(
            " write_coordinates_type2 :: data is saved to sort_index.txt (needed for post process)"
        )
...
def back_convert_cpmd(
    input="IONS+CENTERS.xyz",
    output="IONS+CENTERS_sorted.xyz",
    sortfile="sort_index.txt",
):
    """_summary_
    ...
    for config_num, ase_atom in enumerate(original_xyz):
        ...
        for coord, symbol in zip(
            ase_atom.get_positions(), ase_atom.get_chemical_symbols()
        ):
        ...
        # print(f"cell {cell}")
        sorted_ase_atom = ase.Atoms(
            sorted_atom_list, positions=sorted_coord_list, cell=cell
        )
