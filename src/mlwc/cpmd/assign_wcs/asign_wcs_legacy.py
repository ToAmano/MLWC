...
def relative_vector_torch(atom1, atom2, UNITCELL_VECTORS: np.array):
    ...
    fractional_relative_vec = fractional_relative_vec - torch.round(
        fractional_relative_vec
    )
...
def relative_vectors_torch2(atom1, atom2, UNITCELL_VECTORS):
    ...
    # Apply minimum image convention
    fractional_relative_vecs = fractional_relative_vecs - torch.round(
        fractional_relative_vecs
    )
...
def find_nearest_wfc(bondcenters, wfc_list, UNITCELL_VECTORS, num_wcs: int):
    ...
    nearest_indices = np.argsort(distances, axis=0)[:, :num_wcs]
    # TODO :: indexが重複指定ないかの確認

    # 最も近い2つの `wfc_list` の座標を抽出
    nearest_wfc = wfc_list[nearest_indices]
    # TODO :: indexを返した方が良い説ある．
    return nearest_wfc
...
def calc_bondmu():
    ...
    if wcs_num == 1:
        # 最も近いWCsのインデックスを一つ取り出す．
        wcs_indices = np.argsort(wfc_distances).reshape(-1)[:1]
        mu_lp = (-2.0) * coef * wfc_vectors[wcs_indices[0]]
        # 原子とwcsの距離があまりに遠い場合はWARNINGを出す
        if np.linalg.norm(wfc_vectors[wcs_indices[0]]) > 1.0:
            print(
                "WARNING :: The distance between atom {} and lonepair {} ({}) is too large !! :: {} ".format(
                    atom_coord,
                    wfc_list[wcs_indices[0]],
                    wcs_indices[0],
                    np.linalg.norm(wfc_vectors[wcs_indices[0]]),
                )
            )
        if wcs_indices[0] in picked_wfcs:
            print("WARNING SAME wcs is assined !! :: {}".format(wcs_indices[0]))
    if wcs_num == 2:
        # 最も近いWCsのインデックスを二つ取り出す．
        wcs_indices = np.argsort(wfc_distances).reshape(-1)[:2]
        # 二つのWannierCenterによる双極子モーメントを計算する．
        mu_lp = (
            (-4.0)
            * coef
            * (wfc_vectors[wcs_indices[0]] + wfc_vectors[wcs_indices[1]])
            / 2.0
        )
...
class asign_wcs_torch:
    ...
    def __init__(self, NUM_MOL: int, NUM_MOL_ATOMS: int, UNITCELL_VECTORS):
        ...
        self.UNITCELL_VECTORS = torch.from_numpy(
            UNITCELL_VECTORS.astype(np.float32)
        ).clone()  # 単位胞ベクトル

    # ase_atomsのボンドセンターを計算する
    def aseatom_to_mol_coord_bc(self, ase_atoms: ase.atoms, itp_data, bonds_list: list):
        return raw_aseatom_to_mol_coord_bc(
            ase_atoms, bonds_list, itp_data, self.NUM_MOL_ATOMS, self.NUM_MOL
        )

    def find_all_lonepairs(
        self, wfc_list, atO_list, list_mol_coords, picked_wfcs, wcs_num: int
    ):
        return raw_find_all_lonepairs(
            wfc_list,
            atO_list,
            list_mol_coords,
            picked_wfcs,
    ...
    def find_all_bonds(self, wfc_list, list_bond_centers, picked_wfcs, wcs_num: int):
        return raw_find_all_bonds(
            wfc_list, list_bond_centers, picked_wfcs, self.UNITCELL_VECTORS, wcs_num
        )

    def find_all_pi(self, wfc_list, list_bond_centers, picked_wfcs, double_bonds):
        return raw_find_all_pi(
            wfc_list,
            list_bond_centers,
            picked_wfcs,
            double_bonds,
    ...
    def calc_mu_bond_lonepair(
        self, wfc_list, ase_atoms, bonds_list, itp_data, double_bonds
    ):
        # 実際のﾜﾆｴの割当
        return raw_calc_mu_bond_lonepair(
            wfc_list,
            ase_atoms,
            bonds_list,
            itp_data,
    ...
    def _find_all_lonepairs(
        ...
        # Oのローンペア
        for atOs, mol_coords in zip(
            atO_list, list_mol_coords
        ):  # 分子の数に関するループ
            mu_lpO_mol = []
            wcs_mol = []  # wcsの座標
            for atO in atOs:  # ある分子内の原子に関するループ
                ...
                wcs_indices, mu_lp, wcs_lp = raw_find_lonepairs(
                    center_atom_coord, wfc_list, wcs_num, UNITCELL_VECTORS, picked_wfcs
                )
    ...
    def _find_nearest_lonepairs(
        ...
        if wcs_num == 1:
            wcs_indices = torch.argsort(wfc_distances).reshape(-1)[
                :1
            ]  # 最も近いWCsのインデックスを一つ取り出す．
            mu_lp = (-2.0) * coef * wfc_vectors[wcs_indices[0]]
            # 原子とwcsの距離があまりに遠い場合はWARNINGを出す
...
def raw_aseatom_to_mol_coord_bc(
    ...
    for j in range(NUM_MOL):
        ...
        mol_coords, bond_centers = raw_calc_mol_coord_and_bc_mic_onemolecule_new(
            mol_inds,
            bonds_list,
            ase_atoms[NUM_MOL_ATOMS * j : NUM_MOL_ATOMS * (j + 1)],
            itp_data,
        )  # 1つの分子のmic座標/bond center計算
...
def raw_get_distances_mic(aseatom, a: int, indices, mic=False, vector=False):
    ...
    if mic == True:  # micの時だけdistancesを計算しなおす．
        ...
        cell = aseatom.get_cell()[0][
            0
        ]  # xの座標だけを利用する． # TODO :: 一般の格子に対応させる．
        distances = np.where(
            np.abs(distances) > cell / 2,
            distances - cell * np.sign(distances),
            distances,
        )
    if vector == True:
        return distances
...
def raw_get_pbc_mol(aseatom, mol_inds, bonds_list_j, itp_data):
    ...
    vectors = raw_get_distances_mic(
        aseatom,
        mol_inds[itp_data.representative_atom_index],
        mol_inds,
        mic=True,
        vector=True,
    )
    ...
    bonds_list_from_zero = [
        [i[0] - mol_inds[0], i[1] - mol_inds[0]] for i in bonds_list_j
    ]  # TODO :: bonds_list_jから単に数字をシフトする場合にのみ対応
    ...
    for (
        ...
        bonds_list_from_zero
    ):  # 全てのボンドに対するloop（ボンド間距離が一つでも3 Angstromより大きかったら再計算）
        # ボンド間距離の計算
    ...
    if CALC_FLAG == True:
        print(
            "WARNING(raw_get_pbc_mol) :: mol_index {} :: recalculation of vectors is required.".format(
                mol_inds[0]
            )
        )
        vectors = raw_bfs(
        ...
        for (
            ...
            bonds_list_from_zero
        ):  # 全てのボンドに対するloopでvectorsが正しく再計算されたかチェック
            # ボンド間距離の計算
            bond_distance = np.linalg.norm(vectors[bond[0]] - vectors[bond[1]])
            if bond_distance > 3.0:  # angstroml
                print(
                    " !!ERROR!!: bond distance is too long after BFS modification. bond distance = {0} Angstrom between atom {1}/{2}/{3} and atom {4}/{5}/{6}".format(
                        bond_distance,
                        bond[0],
                        aseatom.get_positions()[bond[0] + mol_inds[0]],
                        aseatom.get_chemical_symbols()[bond[0] + mol_inds[0]],
                        bond[1],
                        aseatom.get_positions()[bond[1] + mol_inds[0]],
                        aseatom.get_chemical_symbols()[bond[1] + mol_inds[0]],
                    )
                )
                print(
                    " vectors[bond[0]]-vectors[bond[1]] {} {}".format(
                        vectors[bond[0]], vectors[bond[1]]
                    )
                )
                print(" ")
                # sys.exit(1)
    ...
    while queue:
        ...
        for near in nears:
            if nodes[near].parent == -1:  # 親ノードが-1なら未探索
                ...
                revised_vector = raw_get_distances_mic(
                    aseatom,
                    node.index + mol_inds[0],
                    nodes[near].index + mol_inds[0],
                    mic=True,
                    vector=True,
                )
...
def raw_calc_mol_coord_and_bc_mic_onemolecule_new(
    mol_inds, bonds_list, aseatoms, itp_data
):
    """
    ...
    # 二つのdrがボンドの両端の原子への距離
    bond_centers = [
        R0 + (vectors[l[0]] + vectors[l[1]]) / 2.0 for l in bonds_list
    ]  # R0にボンドセンターへの座標をたす．
...
def raw_calc_mol_coord_and_bc_mic_onemolecule(
    mol_inds, bonds_list_j, aseatoms, itp_data
):
    """
    TODO :: itp_data.representative_atom_indexを使って書き直す
    # * 系内のあるひとつの分子に着目し，ボンドセンターと（micを考慮した）分子座標を計算する．
    ...
    for l in bonds_list_from_zero:
        # 二つのdrがボンドの両端の原子への距離
        bc = (
            R0 + (vectors[l[0]] + vectors[l[1]]) / 2.0
        )  # R0にボンドセンターへの座標をたす．
...
def raw_calc_bc_mic_onemolecule(mol_inds, mol_coords, bonds_list_j):
    ...
    bond_centers = [
        (mol_coords[l[0]] + mol_coords(l[1])) / 2.0 for l in bonds_list_from_zero
    ]
...
def raw_make_aseatoms_from_wc(atom_coord: np.array, wfc_list, UNITCELL_VECTORS):
    ...
    atom_wan = ase.Atoms(
        atom_id, positions=atom_wcs_coord, cell=UNITCELL_VECTORS, pbc=[1, 1, 1]
    )
    return atom_wan


def raw_find_lonepairs(
    atom_coord: np.array, wfc_list, wcs_num: int, UNITCELL_VECTORS, picked_wfcs
):
    """
    ローンペアまたはボンドセンターから最も近いwcsを探索する
    input
    ...
    wfc_vectors = raw_get_distances_mic_multiPBC(
        atom_wan, 0, range(1, num_element), mic=True, vector=True
    )  # 上のコードと同じことをしている．
...
def raw_find_bondwcs(atom_coord: np.array, wfc_list, wcs_num: int, UNITCELL_VECTORS):
    ...
    if wcs_num == 1:
        wcs_indices = np.argsort(wfc_distances).reshape(-1)[:1]
        mu_lp = (-2.0) * coef * wfc_vectors[wcs_indices[0]]
        # bcとwcsの距離があまりに遠い場合はWARNINGを出す
        if np.linalg.norm(wfc_vectors[wcs_indices[0]]) > 1.0:
            print(
                "WARNING :: The distance between bc {} and wc {} is too large !! :: {} Ang.".format(
                    atom_coord,
                    wcs_indices[0],
                    np.linalg.norm(wfc_vectors[wcs_indices[0]]),
                )
...
def raw_find_pi(
    atom_coord: np.array, wfc_list, r_threshold: float, picked_wcs, UNITCELL_VECTORS
):
    """
    ...
    wfc_distances = np.linalg.norm(wfc_vectors, axis=1)
    wcs_indices = np.argwhere(wfc_distances < r_threshold).reshape(
        -1
    )  # まずは条件に合うのを抽出
...
# * calc_mu_bond_lonepair用の部品関数たち
#
def raw_find_all_lonepairs(
    wfc_list, atO_list, list_mol_coords, picked_wfcs, wcs_num: int, UNITCELL_VECTORS
):
    """
    最後の変数num_wcsでOローンペアとNローンペアに対応
    """
...
def raw_find_all_bonds(
    wfc_list, list_bond_centers, picked_wfcs, UNITCELL_VECTORS, wfc_num: int = 1
):
    """
    ...
    for bcs in list_bond_centers:  # 分子数に関するループ
        ...
        for bond_center_coord in bcs:  # ある分子内のボンドセンターに関するループ
            # wfc_list_exclude_pickedwfcs = [wfc_list[config][i] for i in range(len(wfc_list[config])) if i not in picked_wfcs]
            wcs_indices, mu_bond, wcs_bond = raw_find_lonepairs(
                bond_center_coord, wfc_list, wfc_num, UNITCELL_VECTORS, picked_wfcs
            )
            picked_wfcs = picked_wfcs + list(wcs_indices)
...
def raw_find_all_pi(
    wfc_list, list_bond_centers, picked_wfcs, double_bonds, UNITCELL_VECTORS
):
    ...
    for bcs in list_bond_centers:  # 再度ボンドセンターを探索
        ...
        for dbond_center_coord in dbcs:  # 二重結合的に探索する？
            ...
            # wfc_list_exclude_pickedwfcs = [wfc_list[config][i] for i in range(len(wfc_list[config])) if i not in picked_wfcs]
            wcs_indices, mu_bond, wcs_bond = raw_find_pi(
                dbond_center_coord, wfc_list, r_threshold, picked_wfcs, UNITCELL_VECTORS
            )
...
def raw_calc_mu_bond_lonepair(
    ...
    results = raw_aseatom_to_mol_coord_bc(
        ase_atoms, bonds_list, itp_data, NUM_MOL_ATOMS, NUM_MOL
    )
    ...
    # 各結合上のワニエ中心の座標を取得する
    r_threshold = (
        0.65  # [ang.] 結合中点位置からどの距離までをワニエ中心とみなすかのしきい値
    )
    ...
    list_atomic_nums = list(
        np.array(ase_atoms.get_atomic_numbers()).reshape(NUM_MOL, -1)
    )
    atO_list = [
        np.argwhere(js == 8).reshape(-1) for js in list_atomic_nums
    ]  # 原子番号8
    atN_list = [
        np.argwhere(js == 7).reshape(-1) for js in list_atomic_nums
    ]  # 原子番号7
    ...
    list_mu_lpO, list_lpO_wfcs, picked_wcs_O = raw_find_all_lonepairs(
        wfc_list, atO_list, list_mol_coords, picked_wfcs, 2, UNITCELL_VECTORS
    )
    picked_wfcs = picked_wfcs + list(picked_wcs_O)

    # Nローンペア
    list_mu_lpN, list_lpN_wfcs, picked_wcs_N = raw_find_all_lonepairs(
        wfc_list, atN_list, list_mol_coords, picked_wfcs, 1, UNITCELL_VECTORS
    )
    picked_wfcs = picked_wfcs + list(picked_wcs_N)

    # ボンドセンター
    list_mu_bonds, list_bond_wfcs, picked_wcs_bond = raw_find_all_bonds(
        wfc_list, list_bond_centers, picked_wfcs, UNITCELL_VECTORS, 1
    )
    picked_wfcs = picked_wfcs + list(picked_wcs_bond)

    ########## π電子を探索する###########
    # TODO :: hard code :: ここは改善の余地あり．
    list_mu_pai, list_pi_wfcs, picked_wcs_pi = raw_find_all_pi(
        wfc_list, list_bond_centers, picked_wfcs, double_bonds, UNITCELL_VECTORS
    )
...
def save_yaml(NUM_MOL: int, itpdata, list_bond_centers, list_mu_bonds):
    ...
    for mol_id in range(NUM_MOL):
        ...
        for bond_index, bond in enumerate(itpdata.bonds_list):
            # ボンドindexの座標を計算
            cent_mol = cpmd.descripter.find_specific_bondcenter(
                list_bond_centers, bond_index
            )
            # ボンド双極子
