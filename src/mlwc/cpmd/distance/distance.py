...
class DistanceAbstract(abc.ABC):
    ...
    def compute_distances(cls):
        """abstract method for compute_distances"""


class distance_1d(DistanceAbstract):
    """
...
class distance_2d(DistanceAbstract):
    """
...
class distance_matrix(DistanceAbstract):
    """
    ...
    def compute_distances(
        ...
        distance_vectors = (
            vector_array2[np.newaxis, :, :] - vector_array1[:, np.newaxis, :]
        )
        logger.debug("distance_vectors.shape = %s", np.shape(distance_vectors))
...
class distance_ase(DistanceAbstract):
    """
