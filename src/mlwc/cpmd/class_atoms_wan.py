...
class atoms_wan:
    ...
    def __init__(self, input_atoms: ase.atoms, NUM_MOL_ATOMS: int, itp_data):
        ...
        self.ASIGN = mlwc.cpmd.asign_wcs.asign_wcs(
            self.NUM_MOL, self.NUM_MOL_ATOMS, self.UNITCELL_VECTORS
        )
        self.DESC = mlwc.cpmd.descripter.descripter(
            self.NUM_MOL, self.NUM_MOL_ATOMS, self.UNITCELL_VECTORS
        )
    ...
    def _set_aseatoms_wannier_oneframe(self) -> None:
        ...
        # class として定義
        self.atoms_nowan = ase.Atoms(
            atom_list[atom_list_noX],
    ...
    def aseatom_to_mol_coord_bc(self):
        ...
        list_mol_coords = np.empty(
            [self.NUM_MOL, self.NUM_MOL_ATOMS, 3]
        )  # [] #分子の各原子の座標の格納用
        # [] #各分子の化学結合の中点の座標リストの格納用
        list_bond_centers = np.empty([self.NUM_MOL, len(self.itp_data.bonds_list), 3])
        # 0からNUM_MOL_ATOMSのリスト
        mol_at0 = [i for i in range(self.NUM_MOL_ATOMS)]

        for j in range(self.NUM_MOL):  # 全ての分子に対する繰り返し．
            mol_inds_j = [
                int(at + self.NUM_MOL_ATOMS * j) for at in mol_at0
            ]  # j番目の分子を構成する分子のindex
            bonds_list_j = [
                [
                    int(b_pair[0] + self.NUM_MOL_ATOMS * j),
                    int(b_pair[1] + self.NUM_MOL_ATOMS * j),
                ]
                for b_pair in self.itp_data.bonds_list
            ]  # j番目の分子に入っている全てのボンドのindex
            mol_coords, bond_centers = (
                mlwc.cpmd.asign_wcs.raw_calc_mol_coord_and_bc_mic_onemolecule(
                    mol_inds_j, bonds_list_j, self.atoms_nowan, self.itp_data
                )
