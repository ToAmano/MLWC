...
def plot_norm_from_data(
    ...
    ax.scatter(
        ...
        alpha=0.2,
        s=5,
        label="train RMSE={}".format(rmse0),
    )
    ax.scatter(
        np.linalg.norm(y_pred_valid, axis=1),
        np.linalg.norm(y_true_valid, axis=1),
        alpha=0.2,
        s=5,
        label="test RMSE={}".format(rmse1),
    )
    if limit:
        ax.xlim(0, 4)
        ax.ylim(0, 4)
...
def plot_norm(model, dataset_train, dataset_valid, limit: bool = True):
    ...
    scatter2 = ax.scatter(
        ...
        alpha=0.2,
        s=5,
        label="test RMSE={}".format(rmse_valid),
    )
