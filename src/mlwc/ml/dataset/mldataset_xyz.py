...
class DataSet_xyz(AbstractDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.bondtype == "bond":
            descs_x = self.data[index].DESC.calc_bond_descripter_at_frame(
                self.data[index].atoms_nowan,
            ...
            true_y = self.data[index].DESC.calc_bondmu_descripter_at_frame(
                self.data[index].list_mu_bonds, self.bond_index
            )  # .reshape(-1,3)
            # print(f" SHAPE of DESCS_X = {np.shape(descs_x)}  :: DESCS_Y = {np.shape(true_y)}")
            return (
                torch.from_numpy(descs_x.astype(np.float32)).clone(),
                torch.from_numpy(true_y.astype(np.float32)).clone(),
            )
        elif self.bondtype == "lonepair":
            # !! hard code :: 酸素ローンペアに限定
            descs_x = self.data[index].DESC.calc_lonepair_descripter_at_frame_type2(
                self.data[index].atoms_nowan,
    ...
    def __getitem__(self, index):
        ...
        if self.bondtype == "coc":
            ...
            true_y = self.data[index].DESC.calc_coc_bondmu_descripter_at_frame(
                self.data[index].list_mu_bonds,
                self.data[index].list_mu_lpO,
                self.itp_data.coc_index,
                self.itp_data.bond_index["CO_1_bond"],
            )
            return (
                torch.from_numpy(descs_x.astype(np.float32)).clone(),
                torch.from_numpy(true_y.astype(np.float32)).clone(),
            ...
            true_y = self.data[index].DESC.calc_coh_bondmu_descripter_at_frame(
                ...
                self.itp_data.coh_index,
                self.itp_data.bond_index["CO_1_bond"],
                self.itp_data.bond_index["OH_1_bond"],
            )
            # print(f" SHAPE of DESCS_X = {np.shape(descs_x)}  :: DESCS_Y = {np.shape(true_y)}")
            return (
                torch.from_numpy(descs_x.astype(np.float32)).clone(),
