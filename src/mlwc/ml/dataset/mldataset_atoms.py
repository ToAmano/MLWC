...
class DataSet_atoms(DataSet_abstract):
    """
    ...
    def __init__(self, input_atoms_wan_list: list[atoms_wan], bond_key):
    ...
    def __getitem__(self, index):
        ...
        dict = {  # input for descriptor.forward
            ...
            "atomic_coordinate": torch.from_numpy(atomic_positions.astype(np.float32))
            .clone()
            .requires_grad_(True),
            "UNITCELL_VECTOR": torch.from_numpy(unitcell_vector.astype(np.float32))
            .clone()
            .requires_grad_(True),
            "bond_centers": torch.from_numpy(bc_positions.astype(np.float32))
            .clone()
            .requires_grad_(True),
            "device": "cpu",
        }
...
class ConcreteFactory_atoms(Factory_dataset):
    def create_dataset(self, input_atoms_wan_list: list[atoms_wan], bond_key: str):
        return DataSet_atoms(input_atoms_wan_list, bond_key)
