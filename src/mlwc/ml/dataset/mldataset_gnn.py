...
def prepare_input_tensor(
    ...
    return {
        ...
        "bond_list": to_tensor(
            np.array(data.dict_bonds[bond_key]), dtype=np.int64, requires_grad=False
        ),
    }


class DatasetGNN(AbstractDataset):
    """
...
class ConcreteFactoryGNN(Factory_dataset):
    def create_dataset(self, input_atoms_wan_list: list[atoms_wan], bond_key: str):
        return DatasetGNN(input_atoms_wan_list, bond_key)
