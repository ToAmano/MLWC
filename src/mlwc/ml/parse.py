...
class var_general:
    ...
    def __init__(self, input_general):
        ...
        # print(test)
        path = (
            os.path.dirname(os.path.abspath(__file__)) + "/../data/inputparameter.yaml"
        )
        print(path)
...
class var_descripter:
    ...
    def __init__(self, input_descriptor):
        ...
        self.calc = int(
            decide_if_use_default(find_input(input_descriptor, "calc"), 0)
        )  # 計算するかどうかのフラグ（1がTrue，0がFalse）
        ...
        self.descmode = find_input(
            input_descriptor, "descmode"
        )  # wannier計算をしない（1）かする（2）か
        self.desctype = decide_if_use_default(
            find_input(input_descriptor, "desctype"), "old"
        )  # 記述子の種類
        self.step = find_input(input_descriptor, "step")  # 計算するステップ数(optional)
        self.haswannier = int(
            decide_if_use_default(find_input(input_descriptor, "haswannier"), 0)
        )  # 1がTrue，0がFalse
        self.interval = int(
            decide_if_use_default(find_input(input_descriptor, "interval"), 1)
        )  # trajectoryを何ステップごとに処理するか．デフォルトは毎ステップ．
        self.desc_coh = int(
            decide_if_use_default(find_input(input_descriptor, "desc_coh"), 0)
        )  # # 1がTrue，0がFalse
        self.Rcs = float(
            decide_if_use_default(find_input(input_descriptor, "Rcs"), 4.0)
        )
        self.Rc = float(decide_if_use_default(find_input(input_descriptor, "Rc"), 6.0))
        self.MaxAt = int(
            decide_if_use_default(find_input(input_descriptor, "MaxAt"), 24)
        )
        self.trueonly = int(
            decide_if_use_default(find_input(input_descriptor, "trueonly"), 0)
        )  # trueyだけ計算する場合は1（1がtrue，0がfalse）
...
class var_predict:
    ...
    def __init__(self, input_predict):
        ...
        self.calc = int(
            decide_if_use_default(find_input(input_predict, "calc"), 0)
        )  # 計算するかどうかのフラグ（1がTrue，0がFalse）
        self.model_dir = find_input(input_predict, "model_dir")
        # stdoutfile=find_input(inputs,"stdoutfile")
        self.desc_dir = find_input(input_predict, "desc_dir")  # 記述子のロードdir
        self.modelmode = find_input(
            input_predict, "modelmode"
        )  # normal or rotate (2023/4/16)
        self.bondspecies = int(
            decide_if_use_default(find_input(input_predict, "bondspecies"), 4)
        )  # デフォルトの4はメタノールに対応
        self.save_truey = int(
            decide_if_use_default(find_input(input_predict, "save_truey"), 0)
        )  # 1がTrue，0がFalse（true_yを保存するかどうか．）
        # yamlデータを使う方法
        self.input = {}
