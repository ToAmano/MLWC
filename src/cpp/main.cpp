...
int main(int argc, char *argv[])
{
    ...
    { // NUM_ATOM should be multiple of NUM_MOL_ATOMS
...
#pragma omp parallel for
    // #pragma omp parallel
    for (int i = 0; i < module_load_xyz.NUM_CONFIG; i++)
    {                                                                                      // ここは他のfor文のような構文にはできない(ompの影響．)
        ...
        dipole_frame ch_dipole_frame = dipole_frame(NUM_MOL * test_read_mol.bond_index['CH_1_bond'].size(), NUM_MOL);
        ...
        {
            ch_dipole_frame.predict_bond_dipole_at_frame(module_load_xyz.atoms_list[i], test_bc, test_read_mol.bond_index['CH_1_bond'], NUM_MOL, module_load_xyz.UNITCELL_VECTORS, NUM_MOL_ATOMS, var_des.desctype, module_load_models.module_ch);
            ch_dipole_frame.calculate_wannier_list(test_bc, test_read_mol.bond_index['CH_1_bond']);
        ...
        { // output results in thread=1 to STDOUT file
        ...
        { // 分子に関するループ
            for (int b = 0; b < (int)test_mol[a].size(); b++)
            { // 原子座標
            ...
            for (int b = 0; b < (int)test_bc[a].size(); b++)
            { // ボンドセンター
            ...
            if (module_load_models.IF_CALC_CH)
            {
                for (int b = 0; b < (int)ch_dipole_frame.wannier_list[a].size(); b++)
                { // ch wannier
            ...
            if (module_load_models.IF_CALC_CC)
            {
                for (int b = 0; b < (int)cc_dipole_frame.wannier_list[a].size(); b++)
                { // ch wannier
            ...
            if (module_load_models.IF_CALC_CO)
            {
                for (int b = 0; b < (int)co_dipole_frame.wannier_list[a].size(); b++)
                { // ch wannier
            ...
            if (module_load_models.IF_CALC_OH)
            {
                for (int b = 0; b < (int)oh_dipole_frame.wannier_list[a].size(); b++)
                { // ch wannier
            ...
            if (module_load_models.IF_CALC_O)
            {
                for (int b = 0; b < (int)o_dipole_frame.wannier_list[a].size(); b++)
                { // ch wannier
...
#pragma omp parallel for
        for (int i = 0; i < (int)module_load_xyz.atoms_list.size(); i++)
        { // OMP parallel for frames
