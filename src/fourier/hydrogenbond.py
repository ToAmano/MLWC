...
def calc_oh(traj_liquid: list[ase.Atoms], oxygen_list: list[int], hydrogen_list: list[int]) -> np.ndarray:
    ...
    for counter, atoms in enumerate(traj_liquid):  # frameに関するloop
        ...
        distances = cpmd.distance.distance.distance_2d.compute_distances(
...
def calc_roo(traj_liquid: list[ase.Atoms], oxygen_list: list[int], NUM_MOL: int, NUM_ATOM_ALL: int) -> np.ndarray:
    ...
    # define mask
    n = len(oxygen_list)
    # Start with all pairs being valid (True)
    mask = np.ones((n, n), dtype=bool)

    for i in range(NUM_MOL):
        # [0] is necessary to get the indices
        idx = np.where(oxygen_molecule_ids == i)[0]
        for idx_1 in idx:
            for idx_2 in idx:
                # print(idx_1, idx_2)
    ...
    for counter, atoms in enumerate(traj_liquid):  # frameに関するloop
        pos = atoms.get_positions()
        distances = cpmd.distance.distance.distance_matrix.compute_distances(
        ...
        # Set intra-molecular distances to infinity
