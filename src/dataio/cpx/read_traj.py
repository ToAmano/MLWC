...
class ReadCP(dataio.read_core.custom_traj):
    '''
    ...
    def __init__(self, filename: str):
        super().__init__(atoms_list=raw_transform_xyz(xyz_filename=filename, unitcell_vector=raw_read_unitcell_vector(
            filename), JUMP=False), unitcell_vector=raw_read_unitcell_vector(filename), filename=filename)
    ...
    def save(self, prefix: str = ""):
        if prefix == "":
            ...
            ase.io.write(prefix+"_refine.xyz",
                         self.ATOMS_LIST, format="extxyz")
            # raw_save_aseatoms(self.ATOMS_LIST,  xyz_filename=prefix+"_refine.xyz")
        return 0


class ReadXDATCAR(dataio.read_core.custom_traj):
    '''
    input
    ...
    def __init__(self, filename: str):
        super().__init__(atoms_list=raw_transform_xdatcar(xdatcar_filename=filename),
                         unitcell_vector=raw_transform_xdatcar(xdatcar_filename=filename)[0].get_cell(), filename=filename)
#        self.filename=filename
...
class ReadPOS(dataio.read_core.custom_traj):
    '''
    ...
    def __init__(self, filename: str, pwin: str):
        ...
        # make atoms
        atoms_list = dataio.read_core.raw_make_atomslist(
            pos_list, tmp_cell, tmp_symbol)
        # initialize custom_traj
        super().__init__(atoms_list=atoms_list, unitcell_vector=tmp_cell,
                         filename=filename, time=time_list)
        # pwinも保存
        self.__pwin = pwin
    ...
    def export_dfset_pwin(self, interval_step: int = 100, start_step: int = 0):
        ...
        dataio.read_core.raw_export_dfset(
    ...
    if JUMP == False:
        ...
        for i in np.arange(1, len(atoms_list)):
            ...
            if np.any(np.abs(coord_before-coord) > cell_check/2):
                # print("step :: ", i)
                # jumpする場合には格子定数を追加する．
                tmp1 = np.where(coord-coord_before >
                                cell_check/2, coord, coord+cell_check)
                tmp2 = np.where(coord-coord_before < -
                                cell_check/2, tmp1, tmp1-cell_check)
...
def raw_read_force(for_filename: str):
    ...
    *.forファイルを読みこんでn*3のリストと時間情報を返す.
    ...
    # final step
    for_list.append(block)
    # forceの単位はa.u.=HARTREE ATOMIC UNITS=Eh/bohr=2Ry/bohr (bohr and Ryd/bohr)
    for_list = np.array(for_list)*2
