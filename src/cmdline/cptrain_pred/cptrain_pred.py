...
def mlpred(yaml_filename: str) -> None:
    ...
    atoms_traj: list[ase.Atoms] = ase.io.read(
        input_descriptor.xyzfilename, index=":")
    ...
    logger.info(
        f" The number of atoms in a single frame :: {len(atoms_traj[0])}")
    # * load model
    model_ch = load_model(input_predict.model_dir +
                          '/model_ch.pt', input_predict.device)
    model_co = load_model(input_predict.model_dir +
                          '/model_co.pt', input_predict.device)
    model_oh = load_model(input_predict.model_dir +
                          '/model_oh.pt', input_predict.device)
    model_cc = load_model(input_predict.model_dir +
                          '/model_cc.pt', input_predict.device)
    model_o = load_model(input_predict.model_dir +
                         '/model_o.pt', input_predict.device)
    # below is for coh/coc bindings
    model_coc = load_model(input_predict.model_dir +
                           '/model_coc.pt', input_predict.device)
    model_coh = load_model(input_predict.model_dir +
                           '/model_coh.pt', input_predict.device)
    ...
    for fr_index, fr_atoms in enumerate(atoms_traj):
        # split atoms and wan
        atoms_wan = cpmd.class_atoms_wan.atoms_wan(
        ...
        # calculate bond centers
        list_bond_centers: Float[np.ndarray, "mol bc 3"] = calc_bondcenter(
            list_mol_coords, itp_data.bonds_list)
        ...
        if len(itp_data.bond_index['CH_1_bond']) != 0 and model_ch != None:
            ...
            y_pred_ch = model_ch(X_ch.to(input_predict.device)).to(
                "cpu").detach().numpy()   # 予測 (NUM_MOL*len(bond_index),3)
            # !! ここは形としては(NUM_MOL*len(bond_index),3)となるが，予測だけする場合NUM_MOLの情報をgetできないのでreshape(-1,3)としてしまう．
            y_pred_ch = y_pred_ch.reshape((-1, 3))
            list_wc_coords.append((bond_centers + y_pred_ch/(constant.Ang *
                                  constant.Charge/constant.Debye)/(-2.0)).reshape(NUM_MOL, -1, 3))
            list_wc_symbols.append(100)
            del Descs_ch
            sum_dipole += np.sum(y_pred_ch, axis=0)  # 双極子に加算
            if input_general.save_bonddipole:
                np.savetxt(file_ch_dipole, np.hstack([np.ones((len(y_pred_ch), 1))*fr_index, np.arange(
                    len(y_pred_ch)).reshape(-1, 1), y_pred_ch]), fmt="%d %d %f %f %f")
        ...
        if len(itp_data.bond_index['CO_1_bond']) != 0 and model_co != None:
            ...
            y_pred_co = model_co(X_co.to(input_predict.device)).to(
                "cpu").detach().numpy()
            list_wc_coords.append((bond_centers + y_pred_co/(constant.Ang *
                                  constant.Charge/constant.Debye)/(-2.0)).reshape(NUM_MOL, -1, 3))
            ...
            if input_general.save_bonddipole:
                np.savetxt(file_co_dipole, np.hstack([np.ones((len(y_pred_co), 1))*fr_index, np.arange(
                    len(y_pred_co)).reshape(-1, 1), y_pred_co]), fmt="%d %d %f %f %f")

        if len(itp_data.bond_index['CC_1_bond']) != 0 and model_cc != None:
            bond_centers = np.array(list_bond_centers)[
                :, itp_data.bond_index['CC_1_bond'], :].reshape((-1, 3))
            ...
            X_cc = torch.from_numpy(Descs_cc.astype(
                np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
            y_pred_cc = model_cc(X_cc.to(input_predict.device)).to(
                "cpu").detach().numpy()
            list_wc_coords.append((bond_centers + y_pred_cc/(constant.Ang *
                                  constant.Charge/constant.Debye)/(-2.0)).reshape(NUM_MOL, -1, 3))
            ...
            if input_general.save_bonddipole:
                np.savetxt(file_cc_dipole, np.hstack([np.ones((len(y_pred_cc), 1))*fr_index, np.arange(
                    len(y_pred_cc)).reshape(-1, 1), y_pred_cc]), fmt="%d %d %f %f %f")
        ...
        if len(itp_data.bond_index['OH_1_bond']) != 0 and model_oh != None:
            ...
            X_oh = torch.from_numpy(Descs_oh.astype(
                np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
            y_pred_oh = model_oh(X_oh.to(input_predict.device)).to(
                "cpu").detach().numpy()
            y_pred_oh = y_pred_oh.reshape((-1, 3))
            list_wc_coords.append((bond_centers + y_pred_oh/(constant.Ang *
                                  constant.Charge/constant.Debye)/(-2.0)).reshape(NUM_MOL, -1, 3))
            list_wc_symbols.append(103)
            del Descs_oh
            sum_dipole += np.sum(y_pred_oh, axis=0)
            if input_general.save_bonddipole:
                np.savetxt(file_oh_dipole, np.hstack([np.ones((len(y_pred_oh), 1))*fr_index, np.arange(
                    len(y_pred_oh)).reshape(-1, 1), y_pred_oh]), fmt="%d %d %f %f %f")

        if len(itp_data.o_list) != 0 and model_o != None:
            o_positions = fr_atoms.get_positions()[np.argwhere(
                fr_atoms.get_atomic_numbers() == 8).reshape(-1)]  # o原子の座標
            ...
            X_o = torch.from_numpy(Descs_o.astype(
                np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
            y_pred_o = model_o(X_o.to(input_predict.device)
                               ).to("cpu").detach().numpy()
            y_pred_o = y_pred_o.reshape((-1, 3))
            list_wc_coords.append((o_positions + y_pred_o/(constant.Ang *
                                  constant.Charge/constant.Debye)/(-4.0)).reshape(NUM_MOL, -1, 3))
            list_wc_symbols.append(10)
            del Descs_o
            sum_dipole += np.sum(y_pred_o, axis=0)
            if input_general.save_bonddipole:
                np.savetxt(file_o_dipole, np.hstack([np.ones((len(y_pred_o), 1))*fr_index, np.arange(
                    len(y_pred_o)).reshape(-1, 1), y_pred_o]), fmt="%d %d %f %f %f")
        ...
        if len(itp_data.coc_index) != 0 and model_coc != None:
            ...
            X_coc = torch.from_numpy(Descs_coc.astype(
                np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
            y_pred_coc = model_coc(X_coc.to(input_predict.device)).to(
                "cpu").detach().numpy()
            y_pred_coc = y_pred_coc.reshape((-1, 3))
            del Descs_coc
            sum_dipole += np.sum(y_pred_coc, axis=0)
            if input_general.save_bonddipole:
                np.savetxt(file_coc_dipole, np.hstack([np.ones((len(y_pred_coc), 1))*fr_index, np.arange(
                    len(y_pred_coc)).reshape(-1, 1), y_pred_coc]), fmt="%d %d %f %f %f")
        ...
        if len(itp_data.coh_index) != 0 and model_coh != None:
            ...
            X_coh = torch.from_numpy(Descs_coh.astype(
                np.float32)).clone()  # オリジナルの記述子を一旦tensorへ
            y_pred_coh = model_coh(X_coh.to(input_predict.device)).to(
                "cpu").detach().numpy()
            y_pred_coh = y_pred_coh.reshape((-1, 3))
            del Descs_coh
            sum_dipole += np.sum(y_pred_coh, axis=0)
            if input_general.save_bonddipole:
                np.savetxt(file_coh_dipole, np.hstack([np.ones((len(y_pred_coh), 1))*fr_index, np.arange(
                    len(y_pred_coh)).reshape(-1, 1), y_pred_coh]), fmt="%d %d %f %f %f")
        ...
        np.savetxt(file_mol_dipole, np.hstack([np.ones((len(molecule_dipole), 1))*fr_index, np.arange(
            len(molecule_dipole)).reshape(-1, 1), molecule_dipole]), fmt="%d %d %f %f %f")
        # write atoms to file
        ase.io.write(input_general.savedir +
                     "/mol_wc.xyz", atoms_wc, append=True)
