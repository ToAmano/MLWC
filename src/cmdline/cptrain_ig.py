...
def command_mltrain_ig(args)-> int:
    """mltrain train 
        wrapper for mltrain
    Args:
...
def mlig(model_filename:str, xyz_filename:str, itp_filename:str, bond_name:str)->None:
    """_summary_
        ...
        for data in dataloader_valid:
            ...
            if data[0].dim() == 3: # 3次元の場合[NUM_BATCH,NUM_BOND,288]はデータを整形する
                ...
                for data_1 in zip(data[0],data[1]):
                    ...
                    y = data_1[1]
                    # baseline for ig
                    baseline = torch.zeros(data_1.shape[0], data_1.shape[1])
...
def command_cptrain_ig(args)-> int:
    """mltrain train 
        wrapper for mltrain
    Args:
        args (_type_): _description_
    """
    mlig(args.model,args.xyz,args.mol, args.bond)
    return 0
