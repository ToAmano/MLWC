...
def main():
    ...
    def predict_dipole(fr, desc_dir):
        ...
        # CHボンド
        descs_X_ch = np.loadtxt(desc_dir+'Descs_ch_' +
                                str(fr)+'.csv', delimiter=',')
        # COボンド
        descs_X_co = np.loadtxt(desc_dir+'Descs_co_' +
                                str(fr)+'.csv', delimiter=',')
        # OHボンド
        descs_X_oh = np.loadtxt(desc_dir+'Descs_oh_' +
                                str(fr)+'.csv', delimiter=',')
        # Oローンペア
        descs_X_o = np.loadtxt(desc_dir+'Descs_o_' +
                               str(fr)+'.csv', delimiter=',')
    ...
    # hard code :: 今は4つの結合種があるのでこうしているが，本来はこれではダメ
    num_structure = int(count_csv/4)

    # hard code :: 計算した構造の数 50001
    result_dipole = joblib.Parallel(n_jobs=-1, verbose=50)(
        joblib.delayed(predict_dipole)(fr, desc_dir) for fr in range(num_structure))
    import numpy as np
